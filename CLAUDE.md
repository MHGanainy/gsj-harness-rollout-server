...
├── pyproject.toml               # 0.1.16; wheel force-includes both pins sets, the G2 container capture, ingest_corpus.py + estate.py
├── docs/
...
├── tests/                       # root suite: 176 tests across 9 modules (CI adds corpus 541 — its job label re-set 530 → 541 at CP-106; the runner collects 540 passed + 1 skipped, the demo-thirty pin being local — + mcp-service 164)
...
```
pip install -e ".[dev]"
pytest -q                                  # the root suite: 176 (corpus: .venv/bin/python -m pytest -q estate/corpus/tests → 541)
