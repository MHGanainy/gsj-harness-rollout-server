...
├── tests/                       # root suite: 176 tests across 9 modules (CI adds corpus 494 by its job label — 530 locally since CP-104, whose round-seven module the label, in the unlifted `.github/`, does not yet count (row 112); the demo-thirty pin is local — + mcp-service 164)
...
```
pip install -e ".[dev]"
pytest -q                                  # the root suite: 176 (corpus: .venv/bin/python -m pytest -q estate/corpus/tests → 530)
