...
├── pyproject.toml               # 0.1.14; wheel force-includes both pins sets, the G2 container capture, ingest_corpus.py + estate.py
├── docs/
...
├── tests/                       # root suite: 176 tests across 9 modules (CI adds corpus 494 — 493 + 1 skipped on the runner, the demo-thirty pin is local; the job's label re-set 483 → 494 at CP-102, the release that lifted `.github/` — + mcp-service 164)
