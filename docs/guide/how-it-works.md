...
7. **`postprocess`** (`runtime.download_*` only). pi's transcript and the `out/` deliverable land under `<artifacts_dir>/<session_id>/` — loud but non-fatal: a download that fails here prints and is swallowed, and `postprocess` itself never fails the run. **What can, and did (CP-101, register row 110):** the step that produced the transcript, one stage earlier. pi's command ends `… 2>&1 | tee <session>/logs/agent/pi.txt`, and a pipeline's exit status is its **last** command's — so a `tee` that cannot open its file exits the step non-zero however well pi ran, and step 6's `step <i> exited with code <rc>` becomes `status: ERROR` on a whole trajectory. A containerised gateway without a same-path `TMPDIR` is the way to reach it: [the cure and the mechanism](troubleshooting.md#a-containerised-gateway-and-tmpdir).
