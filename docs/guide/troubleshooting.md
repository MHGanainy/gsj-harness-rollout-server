...
> 2. **Nothing gives an episode container `host.docker.internal`.** Polar's Docker runtime passes only `--network`, never `--add-host`, so any of `estate.clone_url_for`, `estate.mcp_url_base`, `polar.gateway.public_url` written with that name fails from the sandbox. Corrected at CP-99, because "it never resolves" was the wrong reason and cost a stranger 22 minutes when it read as false: on a host whose own `/etc/hosts` carries the name (Docker Desktop's `--add-host=host.docker.internal:host-gateway`, and Desktop's DNS answers it inside nested containers too) the name **resolves and routes nowhere** — which is worse than not resolving, because a dial that times out looks like a network hiccup and a name that resolves looks like an address. Since 0.1.13 `up`'s probe settles it by measurement rather than by rule: it dials each candidate from a container AND from the host and requires its own sentinel's nonce back, so a name that only resolves loses to an address that answers. Use the compose network's own gateway IP instead (`172.28.9.1` on H200, where the retrieval service `0.0.0.0:8790` and gateway `0.0.0.0:8200` bind the host; `172.17.0.1` names the default bridge, not the episodes' network). Find yours: `docker network inspect <runtime.network> --format '{{(index .IPAM.Config 0).Gateway}}'`.
...
Probe `public_url` from both sides. A bare HTTP status proves only that *something* is listening there, not that it is **your** gateway — read the body. That distinction is wishlist row 101: `up`'s own dial accepted any answer through 0.1.12, and since 0.1.13 it binds a sentinel answering a per-run nonce and accepts only the candidate that returns it. This line said "any HTTP status means the gateway answered" until 0.1.13, which is the same reasoning the tool stopped making:
...
| `the retrieval service in gsj-<run>-mcp crashed (its process died; the container still shows running)` — `qemu: uncaught target signal 11` in its log | the amd64 `gsj-mcp-service` image under emulation on an arm64 daemon dies at the embed step (0.3.0 survived it, 0.4.0 does not); build it natively (`docker build --platform linux/arm64 -t gsj-mcp-service:0.5.1-arm64 estate/mcp-service`) and pass `--mcp-image gsj-mcp-service:0.5.1-arm64`. Production is amd64 and runs the shipped image as is |
...
| `no host address reached this probe's sentinel from a container on '…'` — then, per candidate, what answered it from the container and from this host (`… : a foreign listener from the container`; before CP-99: `tried [...] — every one timed out from the container`) | CP-03's one-URL rule has no answer on this host: on Docker Desktop add `127.0.0.1 host.docker.internal` to `/etc/hosts` and pass `--gateway-host host.docker.internal`; `--gateway-host <address>` writes one unprobed (the episode then fails `no completions` if a sandbox cannot dial it — the gateway log shows session polls and no `/v1/chat/completions`). The run is resumable: every phase before the probe is recorded and reused. Since CP-96 the dial runs by `docker exec` inside the run's own retrieval container — nothing is created, nothing can leak; `up --help` says how to choose an address when the probe cannot run |
| `WARNING: the gateway-host probe could not run — timed out after N s via …`, or (CP-99) `— port 8200 is already in use on this host …, so it cannot bind the sentinel a dial has to reach` (CP-96; before it, a bare `subprocess.TimeoutExpired` traceback one file short of `rollout.yaml`, and a `Created` container from the sandbox image left behind on every attempt) | the probe, not the estate, failed: the run continues with the first candidate written **UNMEASURED** and names the candidates it was about to try. Round four measured why: on a copy-on-create storage driver (`vfs`) creating a container from the 731 MiB sandbox image alone outlasts any probe budget. The exec form needs no container; the `docker run` fallback (an adopted retrieval service and no container of the run's own) is named `gsj-probe-<pid>` and removed in a `finally` whatever happens. **Port already in use (CP-99)**: the probe binds a sentinel on the gateway port and accepts only that sentinel's nonce, so a port it cannot bind cannot be measured at all — it says UNMEASURED and names the port rather than measuring whatever else is listening (through 0.1.12 it probed anyway and still printed `measured:`). The usual cause is this run's own gateway already standing: `estate/estate.py status --name <run>`, stop it, re-run `up`. If a sandbox cannot dial the written address, re-run `up` with `--gateway-host <address>` (the run is resumable) |
| `GSJ_PINS_PATH names a pins SKELETON, not pins.` — `found: …/pins.skeleton.json: format 'gsj-pins-skeleton/1'; empty approved sets: […]` | since 0.1.12 (ADR-0042) `up` and `update` refuse, before anything runs, an override that names the skeleton `up` itself writes: unset `GSJ_PINS_PATH` for the first episode (the reference set), inspect its quarantined body, run the page's `derive_my_pins.py` (it reads the skeleton), and name the `pins.gsj.json` it writes. The library's own refusal (`PinsConfigurationError`) fires on first use — the first hash gate a trace reaches, whatever the body's status, after the admission findings: `{}` validated against a skeleton returns `ADM1`/`ADM3` findings and no refusal (CP-104, round seven's b1's near miss; the status clause corrected at CP-105) |
| `WARNING: the tail and the end-of-turn id were NOT measured: POST /tokenize (history_only) answered 400 …` (or `… is not reachable`, `… is not served`) | since 0.1.12 `up` measures G6's tail and `builder.end_of_turn_token_id` from the engine's own render; an endpoint without vLLM's messages-form `/tokenize` (or one not yet serving the model) gets a skeleton whose tail is empty and whose `measured.why` says this, and the library default (151645, the reference model's) stands, said so — measure by hand ([bring-your-own.md#your-model](bring-your-own.md#your-model)) and pass `--end-of-turn-token-id`; re-run `up` once the engine serves and the skeleton is rewritten |
...
| `the Forgejo image … could not be pulled — the transfer was cut mid-layer; re-run the same command.` — `found: unexpected EOF` (or `connection reset`, a `read tcp … i/o timeout`) | the registry answered and the connection dropped while layers were downloading (round seven's b2: nine minutes of `Retrying in N seconds`, then `unexpected EOF`, and the identical `up` completed on its second run); since CP-104 the refusal leads with the same command — the daemon keeps every layer that completed and resumes the rest — and gives the tag/mirror/digest advice only for a second identical failure (through 0.1.14 it gave that advice first, for a missing-manifest cause the error text did not carry) |
...
| `the retrieval service image … is not present on this daemon and could not be pulled` | library 0.1.17 selects `ghcr.io/mhganainy/gsj-mcp-service:0.5.1` from a wheel and the local `gsj-mcp-service:0.5.1` build from a checkout (0.1.8–0.1.16 select 0.5.0); it supports `--decisions-dir`. Registry images are pulled when absent; `--mcp-image` selects a custom or offline loaded image. If `found:` ends in `unexpected EOF` (or a connection reset, a `read tcp` timeout), the transfer was cut mid-layer: since CP-104 the `what to do:` line leads with the same command — the daemon keeps every layer it completed — and the registry and `--mcp-image` advice is for a second identical failure (the Forgejo transfer row above; this 1.28 GiB (measured on 0.5.0) image is the likeliest to be cut). Wheel 0.1.7 defaults to 0.4.1 and needs the explicit published 0.5.0 override for a drop (wishlist 77); wheels 0.1.4–0.1.6 default to 0.4.0 and have no `--decisions-dir`. |
...
| `… is absent on this daemon — pulling it` then nothing for minutes, or `still pulling <image> — Nm elapsed; this host received … / NOTHING` (the CP-94 heartbeat, once a minute) | a large layer on a slow pipe prints nothing until it lands — 21 minutes measured on a healthy pull at ~155 KB/s, 39 for the 1.28 GiB (measured on 0.5.0) retrieval image; `docker pull` prints a line only when a layer changes state. Three checks before you conclude it hung, in order: (1) the heartbeat's layer tally before its byte line: since CP-104 the tally carries its age (`, unchanged for Nm`), and after three unchanged beats a line still receiving bytes adds that the byte count is only the host's — round seven's b1 watched `7/12 layers complete` stand for 24 beats on a pull that completed; a moving tally means slow, not hung, and a frozen one with bytes arriving is undecided, so go on to (2) and (3) (on a remote daemon the byte counters are that host's, not the daemon's); (2) `df --block-size=1M` on the daemon's data root twice a minute apart — it grows as layers land. **Use megabytes, not `df -h`**: at this project's bandwidth `-h` is blind — a stranger measured `47622` → `47777` MB (+155 MB in 60 s) while `df -h` printed `47G` then `47G`, flat for twenty minutes (round five), which is the same false negative for which `docker system df` was retired here (measured flat through an actively progressing pull on Docker 29.8, which accounts an image only once it is complete — round four). And read it as a floor, not a measurement: on a data root you share, `df` counts every tenant — the same stranger nearly filed a 2× disk regression against a correct README because of it; since CP-96 the heartbeat also names the layer phase (`N/M layers complete, K extracting …`) and says when **no bytes are expected** — an extraction moves the disk, not the pipe, and a stranger counted `Download complete` against `Pull complete` by hand to tell the two apart; (3) the two failure signatures (`Retrying in N seconds`/`unexpected EOF`, `failed to extract`/`failed to mount`) — a hung pull eventually prints one on stderr. **`Retrying in N seconds` on its own is NOT one of them**: docker backs off and resumes, and a healthy pull prints it repeatedly — measured on every stranger who pulled in round five (3/3, across five pulls of three distinct layers, every one of which completed; two of them had a hand on Ctrl-C, one 40 s from the end). What makes it a transport failure is the run ENDING in `unexpected EOF`; until then it is the pull recovering, out loud (this sentence said the opposite through 0.1.12 — corrected at CP-99). Only then interrupt `up` (Ctrl-C; exit 130) and re-run it: every phase is idempotent and the daemon resumes from the layers it kept. Do not start a second pull of the same image beside it — a stranger did and measured its own rate drop from 156 to 4 KiB/s |
...
| `… could not be pulled` / `docker compose up … failed` after the daemon downloaded every layer, then `failed to extract layer … whiteout … operation not permitted` or `failed to mount … overlay … invalid argument` | the registry answered — this daemon's storage cannot extract or mount OCI layers (a nested daemon whose data root sits on overlayfs, measured 2026-09-06: `docker info` fine, `docker pull debian:stable-slim` fine, no container starts). Prove it with `docker run --rm alpine true`; fix the daemon (`-v /var/lib/docker` on a nested daemon, or `--storage-driver vfs`), not the registry — `docker save \| docker load` fails on the same layers. Since CP-92 the pull refusals split by where the pull failed — three ways since CP-104: a download failure (the registry not answering, a dial or TLS-handshake timeout) keeps the registry cure, a transfer cut mid-layer (`unexpected EOF`, a reset, a `read tcp` timeout) leads with the same command, and an extraction or mount failure says this |
| `run '…' started and stopped before its record was written` — `found: <run dir> carries this run's ownership marker and no run.json` | an `up` that stopped before `run.json` exists — usually its Forgejo image pull or health wait (round seven's b2, whose `status` then read `no run named codex` for a directory it had called ACTIVE minutes before; since CP-104 the refusal names the phase and the complete resume: `up --name <run> --corpus <corpus root> --runs-dir <runs root>`, every phase idempotent, and `docker ps -a --filter name=gsj-<run>-` for anything the phase created, which the re-run adopts). The `found:` line always names Forgejo, and that is not always true: the run directory is claimed before the corpus is loaded and before the sandbox-image check, so an `up` refused for a missing sandbox image — at +0.1 s, nothing pulled, no container — gets the same answer (measured on 0.1.15, CP-105); read the `up` output you have for the real refusal, and pull that image before the resume |
