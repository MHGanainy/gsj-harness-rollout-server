...
| `run '…' started and stopped before its record was written` — `found: <run dir> carries this run's ownership marker and no run.json` | an `up` cut in its first phase — the Forgejo image pull or its health wait — before `run.json` exists (round seven's b2, whose `status` then read `no run named codex` for a directory it had called ACTIVE minutes before; since CP-104 the refusal names the phase and the complete resume: `up --name <run> --corpus <root> --runs-dir <root>`, every phase idempotent, and `docker ps -a --filter name=gsj-<run>-` for anything the phase created, which the re-run adopts) |
