...
| `PiHarness setup failed (rc=128) on 'git': "fatal: unable to access '…'"` | the harness clones **inside the sandbox** (`git clone --depth 1 --branch timestep-<T> --single-branch`), so `estate.clone_url_for` must resolve from the episode container, not the host. If the message is `Authentication failed` / `could not read Username` (not a DNS/route error), the estate requires sign-in for read and `estate.clone_credential_env` is unset, names an unexported var, or holds a wrong/expired read token (CP-56) — mint it with `estate.sh owner` and export it |
