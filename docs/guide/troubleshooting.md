...
| `… is absent on this daemon — pulling it` then Docker layer progress, or nothing for minutes in a redirected log | `up` inherits Docker's stdout: a terminal shows native layer download/extraction progress; redirected output reports layer state changes and can remain quiet while a large layer transfers — 21 minutes measured on a healthy pull at ~155 KB/s, 39 for the 1.28 GiB (measured on 0.5.0) retrieval image. The application adds no pull heartbeat or host-byte tally. Three checks before you conclude it hung, in order: (1) Docker's layer states and, in a terminal, byte/extraction progress. A quiet redirected log alone does not establish whether the pull is stalled; go on to (2) and (3); (2) `df --block-size=1M` on the daemon's data root twice a minute apart — it grows as layers land. **Use megabytes, not `df -h`**: at this project's bandwidth `-h` is blind — a stranger measured `47622` → `47777` MB (+155 MB in 60 s) while `df -h` printed `47G` then `47G`, flat for twenty minutes (round five), which is the same false negative for which `docker system df` was retired here (measured flat through an actively progressing pull on Docker 29.8, which accounts an image only once it is complete — round four). And read it as a floor, not a measurement: on a data root you share, `df` counts every tenant — the same stranger nearly filed a 2× disk regression against a correct README because of it; extraction itself can leave the network quiet while other layers may still download; Docker's `Extracting`, `Download complete` and `Pull complete` states distinguish the phases; (3) a pull that ends in `unexpected EOF` or `failed to extract`/`failed to mount`: stderr is captured for the application's existing failure diagnosis. **`Retrying in N seconds` on its own is NOT one of them**: docker backs off and resumes, and a healthy pull prints it repeatedly — measured on every stranger who pulled in round five (3/3, across five pulls of three distinct layers, every one of which completed; two of them had a hand on Ctrl-C, one 40 s from the end). What makes it a transport failure is the run ENDING in `unexpected EOF`; until then it is the pull recovering, out loud (this sentence said the opposite through 0.1.12 — corrected at CP-99). Only then interrupt `up` (Ctrl-C; exit 130) and re-run it: every phase is idempotent and the daemon resumes from the layers it kept. Do not start a second pull of the same image beside it — a stranger did and measured its own rate drop from 156 to 4 KiB/s |
