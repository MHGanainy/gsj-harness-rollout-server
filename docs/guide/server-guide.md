...
Five hard invariants the validator enforces: (1) `timestep-T/pages/` physically holds exactly pages `1..T`; (2) numbering is absolute and 4-digit (`^page_(\d{4})\.md$`); (3) pages shared between timesteps are byte-identical (sha256 — fix a typo in *every* timestep that holds the page); (4) `prompts.yaml` entries are `{id, source, name}` (`skill:<name>` resolving to a non-empty `skills/<name>/SKILL.md`) or `{id, source, text}` (`free:<slug>`); (5) a case sits under `train/` or `eval/`, never both. `corpus.yaml` essentials: `owner` is `gsj-staging`|`gsj-prod` (selects the credential variable), `forgejo.base_url` (with `file://<path>` first-class — no API, no token), optional `mcp.url_base` (absent skips `ingest`), fixed `git.name`/`email`/`date` for deterministic SHAs, `sandbox_image` riding every row.
