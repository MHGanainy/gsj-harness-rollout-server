...
The server role needs Polar's two processes and a running estate. The PyPI wheel ships no `vendor/`, so Polar comes one of two ways — the published image `ghcr.io/mhganainy/gsj-polar:f0e8343a-gsj0.1.17` (public, anonymously pullable; the two `docker run` invocations, one clause per flag, are [bring-your-own.md#polars-two-processes](bring-your-own.md#polars-two-processes)) or the checkout below with its own `uv` venv — and since CP-104 `up`'s closing block names the image in full (round seven's two library-door strangers found this page, `troubleshooting.md` and `serve`'s NOTE still saying "checkout" while the bring-your-own page said "image"; `serve`'s NOTE still does — register row 108). The checkout route:
...
| `builder.end_of_turn_token_id` | int | `151645` | `<\|im_end\|>` under the served Qwen3 tokenizer; since 0.1.12 `up` measures it from the endpoint's own `/tokenize` render on every run (re-running `up` re-derives it when `estate.model` changes; recorded with the G6 tail in `pins.skeleton.json`) and this default stands only when nothing could be measured; `up --end-of-turn-token-id` overrides the measurement, and an explicit value persists across re-runs of the same `--name` — [bring-your-own.md#your-model](bring-your-own.md#your-model) |
...
Files are `<session_id>.<pins_mode>.json` in `traces_dir/` or `quarantine/`. `<pins_mode>` is the pins file's `mode` key, read once at construction: `thinking-off` (no key — the reference set), `thinking-on`, any string matching `[A-Za-z0-9][A-Za-z0-9_-]{0,31}`, or `pins-unresolved` when the file is unreadable — that last one is a misconfiguration: fix `GSJ_PINS_PATH` and restart, both legs. **Admission precedes pins resolution** (CP-104, round seven's b1): `validate_session_result` runs ADM1–ADM4 before any approved set is read, so an operator whose `GSJ_PINS_PATH` is wrong *and* whose first callback carries no trace to check (a `null` `trajectory`, or an empty `traces`) sees an ordinary rejection (`ADM3`, or `ADM4` with G7's), not the 500 — a body with no `trajectory` key at all never gets that far: the receiver answers it `400` — any body that carries a trace, `ERROR` or not, still raises at its first hash gate (measured on the library at CP-105), so the receiver answers 500 — and the `pins-unresolved` suffix on the file name is the tell, and it outranks the findings inside. An accepted file is the POSTed `SessionResult` verbatim; a quarantined file wraps it under `session_result` with the `findings` list first — forensics, not a queue: nothing ever re-reads or promotes it. The finding vocabulary is in [validation-and-pins.md](validation-and-pins.md). Embedding: `Receiver(host, port, traces_dir, quarantine_dir)` has no signal handlers; port `0` binds ephemeral, read `.port` back; `receiver.ingest(body)` is the handler's code path.
...
One command instead of the sequence below: `estate/estate.py up` (or `./estate.sh bringup up`; from the wheel, no clone: `pip install gsj-harness-rollout-server pyarrow` then `python -m gsj_rollout.estate up --corpus <root>` — pyarrow is the taskbank's writer and `up` refuses without it) validates the corpus, creates **or adopts** Forgejo and the retrieval service, runs the five pipeline phases, probes the engine, measures the G6 tail and the end-of-turn id from its own render (since 0.1.12) and writes a validated `estate/runs/<name>/rollout.yaml` with every secret in a `0600` `.env` and the pins skeleton — `estate/runs/<name>/pins.skeleton.json` — beside it — [estate/README.md](https://github.com/MHGanainy/gsj-harness-rollout-server/blob/main/estate/README.md#estatepy--corpus-to-estate-in-one-command) has the prompts, the adopt paths and the re-run posture. Since CP-73 the interactive prompts say which answers bind (the owner, the run name, the embedding identity once a store is built) and which do not (the engine URL and model land in `rollout.yaml` and change by editing it or re-running), the retrieval config is printed for review **before** the index is built under it (each setting priced by what a later change costs; `--mcp-config <yaml>` is the scripted form), and **`estate.py update --name <run>`** syncs corpus edits into the standing estate — diff against the run's lock, report, then push only the changed repos, rebuild the bank, one if-stale reindex, verify.

**`status --name <run>` has three states since CP-94** (wheels from 0.1.11, and a checkout), and since CP-104 (wheels from 0.1.15) a fourth answer for a run that never wrote its record. **ACTIVE**: a command holds the run's kernel lock (`<runs-dir>/.locks/<name>.lock`, held by `up`, `update` and `down` for their whole command and dropped with the process) — `status` says so, prints the phases that stand, says *wait*, and exits 0; it never recommends a second `up` (which the lock would refuse as busy). Three round-three strangers ran `status` against a run that had not died and were told it had, or that nothing was running. **Incomplete**: the lock is free and the record stops short — the report below, now corrected against the daemon (the record's `mcp` block lands only after ingest, the engine and config blocks with the closing write, so a run that stopped between the retrieval service's start and the end used to read `mcp not reached` two lines above `gsj-<run>-mcp Up`; `status` asks `compose ps` and `/health` now, and says `not recorded` where the record cannot know). **Stopped before its record** (CP-104): the lock is free and the directory holds this run's `.estate-run` marker but no `run.json` — an `up` that stopped before its first record write (round seven's b2 lost its Forgejo pull to `unexpected EOF` there); `status` answers `run '<name>' started and stopped before its record was written` with the complete resume (`up --name <run> --corpus <corpus root> --runs-dir <runs root>`), where through 0.1.14 it said `no run named`. **Complete**: the standing report. Also since CP-94: `up` checks the sandbox image **before** any container is pulled or created (a stranger's `up` ran 41 minutes of pipeline and then met the refusal for an image whose name it printed at +0.0s; the refusal text is unchanged), prints a heartbeat once a minute during an image pull (elapsed, and whether this host received bytes — a large layer on a slow pipe prints nothing until it lands: 21 and 39 minutes measured), names the approved sets the pins in force leave EMPTY, and prints wheel-aware Polar commands in its closing block (a bare `polar` is on nobody's PATH from a wheel: the two Polar processes need the published image — spelled in full in the NOTE since CP-104, and run on the default `--polar-leg host` by [bring-your-own.md#polars-two-processes](bring-your-own.md#polars-two-processes)'s two `docker run` invocations — or a checkout's `vendor/polar` venv; `--polar-leg container` is the demo's compose shape). **A run whose `up` died mid-phase** (a failed image pull after Forgejo stood, say) keeps what its phases created — `.estate-run`, `.env`, `compose.yaml`, the lock, the containers and the network so far. Since CP-92 `status --name <run>` on such a run first reports the phases that completed and the services that stand (Forgejo's health, the tokens, the pushed repos), then the cure: re-run `up --name <run> --corpus <the run's corpus root>` to resume (`reused:` lines; the corpus is asked before the record is read, and the printed cure carries the recorded path), or `down` to stop what it created.

**The readiness probe and the clocks (CP-96; wheels from 0.1.12, and a checkout).** `up`'s last measurement — the gateway host a sandbox can dial — used to `docker run --rm` the 731 MiB sandbox image under a 120 s budget; on a copy-on-create daemon (`vfs`) the create alone outlasts it, and the failure was a bare `subprocess.TimeoutExpired` one file short of `rollout.yaml` with a `Created` container left behind (round four, both codebases, four times). The dial now runs by `docker exec` inside the run's own retrieval container (nothing created, nothing leaked), a probe that cannot run degrades to an **UNMEASURED** first candidate naming `--gateway-host` and the candidates it was about to try, and `--help` says how to choose one. Every readiness wait's budget (`--ingest-timeout`, Forgejo's 120 s) is spent on the process's monotonic clock — a suspended host no longer converts a night into an instant refusal on wake — and the refusal prints what it waited beside what it budgeted; the poll line names the collection the service is building (`building decisions: batch i/N`) and repeats itself once a minute while nothing changes.

**The pins skeleton (CP-97, ADR-0042; wheels from 0.1.12).** `up` writes `<run>/pins.skeleton.json` beside `rollout.yaml`: `tool_roster_hash` and `settings_hash` carried from the pins in force, `g6_expected_tail_ids` and the end-of-turn id measured from the engine's own render under the kwargs pi sends for the run's `--thinking` level (the [#your-model](bring-your-own.md#your-model) measurement, performed by `up` — every request/response pair is kept in the file), `skill_card_hash` and `system_prompt_hash` empty because only an inspected quarantined episode supplies them, G4's two empty, `not_measured` and `coverage` stated. `builder.end_of_turn_token_id` in `rollout.yaml` takes the measured id unless `--end-of-turn-token-id` says otherwise (an explicit value persists across re-runs; a disagreement is warned about); an endpoint whose `/tokenize` cannot render its template gets a skeleton that says why, and the library default stands, said so. The file is **not pins**: its `format` is `gsj-pins-skeleton/1`, `GSJ_PINS_PATH` pointed at it makes the receiver refuse every trace that reaches a hash gate with a 500 naming the empty key and the file (`PinsConfigurationError`), and `up`/`update` refuse such an override before anything runs. **"First use" is the first hash gate a trace reaches, whatever the body's status.** The admission findings (ADM1–ADM4 — status, trajectory, builder findings, traces) are computed before any pins are read, so a body with no trace to check (`{}`, no `trajectory`, or an empty `traces`) validated against the skeleton comes back as findings, not a refusal — while any body that carries a trace raises, `ERROR` or `COMPLETED` (measured on the installed 0.1.15 at CP-105; CP-104 wrote "a `COMPLETED` body"); round seven's b1 tested `{}` first, read that as two broken promises for a minute, re-ran with a real accepted body and got the `PinsConfigurationError` above (CP-104). Test the refusal with a body that carries a trace. [bring-your-own.md#your-pins](bring-your-own.md#your-pins)'s `derive_my_pins.py` reads it, so the walk starts from what was measured and still derives the two.
...
A corpus that carries its own `decisions/` (corpus-contract v3, CP-88, published in 0.1.9 — validated with the tree, locked in `decisions.lock.json`) is mounted by default; `--decisions-dir <host directory>` is the override for a drop kept outside a corpus, and a corpus `decisions/` together with that flag — given now or recorded by an earlier `up` of the run — is refused, naming both and that the corpus's wins (pass `--decisions-dir ''` to drop a recorded override). `decisions.path` in the overrides file is refused because the estate owns that mount. The review names the drop's source and still prices each setting's consequence; `verify` compares the served drop against the lock. Default store identity is unchanged, including the absent-when-32 batch-size fingerprint convention.
...
Secret files may contain one optional terminal LF as file framing; CRLF, multiple LFs and embedded line separators are refused, and no spaces are trimmed. `printf '%s'` writes an exact value without file framing. The estate validates legacy env files before loading values so a separator cannot silently shorten a credential. A refused stored line must be repaired **by hand** to the printed `KEY='value'` form with a compatible credential and LF framing before `up` can re-adopt from the named environment variable or credential file; exporting a replacement alone cannot bypass that refusal. The printed alternative is `down --wipe` and a fresh run, subject to verified teardown. Submit's existing environment-first, adjacent-`.env` reader stays unchanged.
...
- **Nothing gives an episode container `host.docker.internal`** — Polar's Docker runtime passes only `--network`, never `--add-host`. Address host-bound services by the compose network's gateway IP, `172.28.9.1` (not `172.17.0.1`). Reworded at CP-99: the old wording said the name *never resolves*, and a stranger on a nested daemon watched it resolve (its outer host's `/etc/hosts` line, served on into nested containers by Docker Desktop's DNS) and route nowhere — a name that resolves is not an address that answers, and the difference cost that run its episode.
- **One `public_url`, two dialers.** The rollout API dispatches to it from the host and pi dials it + `/v1` from the container — never `localhost`, and its port must equal `polar.gateway.port`. `estate.py` *measures* the host rather than deriving it — and since CP-99 the measurement is worth its name: it binds a sentinel on the gateway port that answers a **nonce**, dials every candidate from a container on the run's network **and from this host**, and accepts only the candidate that returns that nonce on both legs. Before CP-99 any HTTP answer passed, `host.docker.internal` went first on nothing but resolving, and a port already in use was probed against somebody else's listener — which is how one stranger's `up` wrote a dead address labelled `measured: dialable`. The label now names what was measured; where the port cannot be bound it says UNMEASURED and why. (On a Docker Desktop Mac the LAN IP was host-only and `host.docker.internal` container-only — CP-59, which is why nothing here is a heuristic.)
...
- **The exemptions are fixed** — `search_decisions`/`decision_stats` exempt, `case_status` reports the token's scope. `search_decisions` answers with one JSON object, `{query, k, hits, index_commit}` (the decisions surface, [`decisions-surface.md`](../decisions-surface.md) §7) — level-2 hits with `rn` and `section` over a drop the estate mounts (the corpus's `decisions/` by default from 0.1.9, or an external drop through `estate.py up --decisions-dir <dir>`; wheels 0.1.8–0.1.16 select the published 0.5.0 image; 0.1.17 selects 0.5.1 for fresh runs (the local build in a checkout, the published index in a wheel). On wheel 0.1.7, pass that image explicitly with `--mcp-image` — wishlist 77), the synthetic 30 otherwise; a consumer keys on the tool's name, never on a `page` key.
