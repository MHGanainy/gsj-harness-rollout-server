...
**`status --name <run>` has three states since CP-94** (wheels from 0.1.11, and a checkout). **ACTIVE**: a command holds the run's kernel lock (`<runs-dir>/.locks/<name>.lock`, held by `up`, `update` and `down` for their whole command and dropped with the process) — `status` says so, prints the phases that stand, says *wait*, and exits 0; it never recommends a second `up` (which the lock would refuse as busy). Three round-three strangers ran `status` against a run that had not died and were told it had, or that nothing was running. **Incomplete**: the lock is free and the record stops short — the report below, now corrected against the daemon (the record's `mcp` block lands only after ingest, the engine and config blocks with the closing write, so a run that stopped between the retrieval service's start and the end used to read `mcp not reached` two lines above `gsj-<run>-mcp Up`; `status` asks `compose ps` and `/health` now, and says `not recorded` where the record cannot know). **Complete**: the standing report. Also since CP-94: `up` checks the sandbox image **before** any container is pulled or created (a stranger's `up` ran 41 minutes of pipeline and then met the refusal for an image whose name it printed at +0.0s; the refusal text is unchanged), prints a heartbeat once a minute during an image pull (elapsed, and whether this host received bytes — a large layer on a slow pipe prints nothing until it lands: 21 and 39 minutes measured), names the approved sets the pins in force leave EMPTY, and prints wheel-aware Polar commands in its closing block (a bare `polar` is on nobody's PATH from a wheel: the two Polar processes need a checkout's `vendor/polar` venv, `--polar-leg container` the demo's image shape). **A run whose `up` died mid-phase** (a failed image pull after Forgejo stood, say) keeps what its phases created — `.estate-run`, `.env`, `compose.yaml`, the lock, the containers and the network so far. Since CP-92 `status --name <run>` on such a run first reports the phases that completed and the services that stand (Forgejo's health, the tokens, the pushed repos), then the cure: re-run `up --name <run> --corpus <the run's corpus root>` to resume (`reused:` lines; the corpus is asked before the record is read, and the printed cure carries the recorded path), or `down` to stop what it created.
...
**The pins skeleton (CP-97, ADR-0042; wheels from 0.1.12).** `up` writes `<run>/pins.skeleton.json` beside `rollout.yaml`: `tool_roster_hash` and `settings_hash` carried from the pins in force, `g6_expected_tail_ids` and the end-of-turn id measured from the engine's own render under the kwargs pi sends for the run's `--thinking` level (the [#your-model](bring-your-own.md#your-model) measurement, performed by `up` — every request/response pair is kept in the file), `skill_card_hash` and `system_prompt_hash` empty because only an inspected quarantined episode supplies them, G4's two empty, `not_measured` and `coverage` stated. `builder.end_of_turn_token_id` in `rollout.yaml` takes the measured id unless `--end-of-turn-token-id` says otherwise (an explicit value persists across re-runs; a disagreement is warned about); an endpoint whose `/tokenize` cannot render its template gets a skeleton that says why, and the library default stands, said so. The file is **not pins**: its `format` is `gsj-pins-skeleton/1`, `GSJ_PINS_PATH` pointed at it makes the receiver refuse every trace that reaches a hash gate with a 500 naming the empty key and the file (`PinsConfigurationError`), and `up`/`update` refuse such an override before anything runs. **"First use" is the first hash gate a `COMPLETED` body reaches.** The admission gates (ADM1–ADM4 — status, trajectory, builder findings, traces) run before any pins are read, so an empty or malformed body validated against the skeleton comes back as `ADM1`/`ADM3` findings, not a refusal; round seven's b1 tested `{}` first, read that as two broken promises for a minute, re-ran with a real accepted body and got the `PinsConfigurationError` above (CP-104). Test the refusal with a body that passes admission. [bring-your-own.md#your-pins](bring-your-own.md#your-pins)'s `derive_my_pins.py` reads it, so the walk starts from what was measured and still derives the two.
