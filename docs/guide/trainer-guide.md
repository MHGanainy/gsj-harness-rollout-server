...
| `gsj_rollout/estate.py` | corpus → estate with no clone: `python -m gsj_rollout.estate up --corpus <root>` — needs Docker and `pip install pyarrow` (the taskbank's parquet writer; `up` refuses without it, naming the cure); a non-reference model's served name, end-of-turn id (`--end-of-turn-token-id`) and G6 tail come from [bring-your-own.md#your-model](bring-your-own.md#your-model); runs land in `./runs/<name>/` (`--runs-dir` puts them elsewhere). Seven verbs: `scaffold` (an annotated starting corpus), `validate`, `up`, `ingest`, `update` (sync an edited corpus into a standing estate), `status`, `down`. History: wheels 0.1.3–0.1.5 carry this file as `gsj_rollout/bringup.py` (`python -m gsj_rollout.bringup up`); 0.1.4 (CP-62) added `--runs-dir` and the image pulls (`--forgejo-image` / `--mcp-image` name others) — 0.1.3's bring-up pins a Forgejo tag codeberg no longer serves and never pulls the retrieval image; 0.1.6 (CP-71–73) is the rename, the folded `validate`/`ingest`, `scaffold`, and `update`; 0.1.7 (CP-79) adds `up --decisions-dir <dir>` — a rii-dok v1 drop mounted read-only into the retrieval service and served as Randnummern (level 2 of [`docs/decisions-surface.md`](https://github.com/MHGanainy/gsj-harness-rollout-server/blob/main/docs/decisions-surface.md)), named in the pre-embed review and the run record — on 0.1.7 this needs `--mcp-image ghcr.io/mhganainy/gsj-mcp-service:0.5.0`; 0.1.8 defaults to that published image and includes the CP-83/84 run, teardown, config and credential boundary repairs; 0.1.9 adds corpus-contract v3 (`decisions/`, its lock and default mount) and CP-90's refusal repairs, including an unwritable runs root and an actionable legacy-credential cure; 0.1.10 (CP-92) makes `status` report the phases that stand before it refuses an incomplete run, splits the pull refusal into download and extract/mount causes and prescribes the native-platform pull for the sandbox image; 0.1.11 (CP-94) gives `status` its three states (ACTIVE via the run lock, incomplete corrected against the daemon, complete), checks the sandbox image before any container is pulled or created, prints a heartbeat during image pulls, names the approved sets left empty on `up`'s pins line, and adds help on the four bare `up` flags and `--runs-dir`; 0.1.12 (CP-96 + CP-97) runs the gateway-host probe by `docker exec` into the run's own retrieval container and degrades instead of aborting when the probe cannot run, spends every readiness budget on the process's clock and prints what it waited beside what it budgeted, names the collection the retrieval service is building, counts `verify`'s skips apart from its passes, names the pull's layer phase in the heartbeat, warns on a copy-on-create storage driver, and writes `<run>/pins.skeleton.json` beside the run's own `rollout.yaml` (on the demo door that run directory is `work/runs/demo/`, not `work/estate/` — CP-99: a stranger looked in the wrong one first, and the estate writes three `rollout.yaml`) — the G6 tail and the end-of-turn id measured from the endpoint's own render, the carried sets, the derived sets empty, refused as pins by the library and by `up` (ADR-0042); `builder.end_of_turn_token_id` takes the measurement unless `--end-of-turn-token-id` says otherwise; 0.1.13 (CP-98 + CP-99) makes the gateway-host probe's label mean what it says — the probe binds a sentinel that answers a per-run nonce and accepts only the candidate returning it from a container on the run's network AND from this host (a foreign listener no longer passes, and `run.json`'s `gateway_host_probe` records what answered each leg), `host.docker.internal` is appended rather than tried first on nothing but resolving, a gateway port this process cannot bind is reported UNMEASURED rather than measured against somebody else's listener, a `docker exec` that could not start the interpreter is a failure rather than silence (exit 127 writes to stdout), a measurement reused from the record no longer says `measured`, the `vfs` warning carries the numbers actually measured, the pins skeleton says when the pins in force already cover the corpus, and the fallback probe's reaper reads *removed* off the CLI's stdout so its 30 s bound engages — and because the dial now requires this run's own nonce back on both legs, an `up` that chose `host.docker.internal` at 0.1.12 may choose a different `polar.gateway.public_url` at 0.1.13 on an unchanged host: that is the fix, not a regression; 0.1.14 (CP-101) makes `up`'s `== run <name> ==` footer conditional on the same fact its pins line already tests — on an estate whose pins in force cover the corpus it prints `G1/G2 EMPTY in it — NOT NEEDED on this estate: <pins path> already carries them` where 0.1.13 printed `G1/G2 EMPTY until an inspected quarantined episode supplies them` unconditionally, twenty lines under a pins line saying the opposite (a round-six stranger filed the pair); on a corpus the pins do not cover the 0.1.13 sentence is unchanged byte for byte, and the `== run <name> ==` markers do not move; 0.1.15 (CP-104) is round seven's: the closing block names the published Polar image with the page whose two `docker run` invocations start its processes, lists the rows `submit --row N` addresses (the first twelve, then how many more), and prints `status`/`down` with `--runs-dir` and the interpreter that ran `up`; `status` lists the rows too, and names a run that stopped before its record as such, with the complete resume; the pull heartbeat carries the layer tally's age; a transfer cut mid-layer is its own refusal whose cure is the same command; the generated `rollout.yaml` header carries the complete re-run; a `/tokenize` request with no answer is retried once and reported as measured; and the skeleton's own prose says ADM1–ADM4 run before any pins are read — the `== run <name> ==` header and the `next — ` line again unchanged. At 0.1.14 and at 0.1.15 `gsj_rollout/` is byte-identical to the release before but for the version literal — what changed for a library-door operator at 0.1.14 is on the pages ([bring-your-own.md#polars-two-processes](bring-your-own.md#polars-two-processes) and [troubleshooting.md#a-containerised-gateway-and-tmpdir](troubleshooting.md#a-containerised-gateway-and-tmpdir)), which reach a reader at the push, not the release |
