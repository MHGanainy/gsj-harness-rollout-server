...
| A-28 | the demo estate's published images track the library release: `ghcr.io/mhganainy/gsj-polar` (Polar at `POLAR_SHA` from the public repo's vendored, patched tree + the PyPI wheel, one interpreter — tag encodes both, `f0e8343a-gsj0.1.2`) and `ghcr.io/mhganainy/gsj-mcp-service` (the mcp-service image as shipped, `0.3.0`) are rebuilt/republished, **multi-arch**, whenever a release changes what they carry — added at CP-34 (demo-repo external ADR-0001) | the demo repo (`gsj-rollout-demo`) is built ONLY from published artifacts (PyPI wheel, GHCR images, the public library repo at a pinned ref) — nothing of ours by path; the images are therefore load-bearing artifacts of the consumer surface exactly like the wheel. Polar's dependency bounds are floors (no lockfile), so each image build resolves them fresh and the image tag IS the effective pin. Single-platform builds measured fatal at the CP-34 smoke: an Apple-Silicon build died `exec format error` on the amd64 estate box — hence multi-arch is part of the assumption, not a nicety | a stale `gsj-polar` image serves an old wheel to every demo estate silently (the bootstrap pins the tag, so it fails STALE, not loud); the cure is mechanical — rebuild with `LIB_REF`/`LIB_VERSION` at the new release and re-push — but nothing enforces it yet; wishlist row 34's visibility flip and any future release checklist item are where it becomes enforced. **[CP-51, audit W3] QUALIFIED**: "multi-arch" holds for `gsj-polar` only — `gsj-mcp-service:0.3.0` and `gsj-pi-harness:pi0.83.0-3` publish linux/amd64 only (F-54, CP-36; live manifest probe 2026-08-27 unchanged, no newer tags); ARM hosts pull `--platform linux/amd64` (the demo bootstrap's cure). The intent stands; the fact was false when written. ~~`waiting-on: the operator republish wishlist row 40 names — nothing schedules one`~~ (occurred: the retrieval image at CP-61 — `gsj-mcp-service:0.4.0` two-platform, the CP-61 report — and the harness at CP-64; struck at CP-98) **[CP-61]** `gsj-polar` rebuilt multi-arch at `LIB_REF=v0.1.3`/`LIB_VERSION=0.1.3` — tag `f0e8343a-gsj0.1.3`, index `ae64cf2c…`, `issubclass(PiHarness, BaseHarness)` and `import gsj_rollout.bringup` verified on both platforms; `gsj-mcp-service:0.4.0` published two-platform (wishlist 40 [CP-61]). "Multi-arch" now holds for two of the three images; the harness image remains the qualifier. ~~`waiting-on: the harness republish wishlist row 40 names — nothing schedules one`~~ (occurred at CP-64: `pi0.83.0-3` index `1fc083d4…`, amd64 `864dc885…` unchanged + arm64 `1dd08d75…` — the CP-64 report; struck at CP-98) **[CP-64]** `gsj-pi-harness:pi0.83.0-3` published two-platform (wishlist 40 [CP-64], CLOSED): the index composed from the measured amd64 child `864dc885…` (byte-unchanged — the H200's frozen load and prior provenance stand) and a fresh arm64 build from the archived predecessor's `sandbox/`, pins-walk-verified with one episode per platform (G2/G3 reproduced on both). **Multi-arch now holds for all three images**; both earlier tokens' events (the operator republish, the harness republish) occurred here. **[CP-81] The rebuild duty honoured for 0.1.7, and the retrieval image moved with it — the token's event occurred.** `ghcr.io/mhganainy/gsj-polar:f0e8343a-gsj0.1.7` built two-platform from the demo's own `estate/polar.Dockerfile` (`LIB_REF=v0.1.7`, `LIB_VERSION=0.1.7`) and pushed: index `sha256:98361f0a…`, `linux/amd64` + `linux/arm64`, the index and both children fetched with an anonymous registry token, and a plain `docker pull` on this arm64 workstation resolving to an image that reports `gsj_rollout 0.1.7` with CP-75's `_named_value` in its packaged `config.py`. The demo's other two answers moved in the same commit: `MCP_IMAGE` `0.4.0` → `0.5.0` (the decisions surface — `0.4.x` refuses the `decisions.path` key a drop needs, wishlist 77 measured at CP-81) and `SANDBOX_IMAGE` unchanged at `pi0.83.0-3`. Three of the demo's four images are therefore current with 0.1.7; the assumption's mechanical half worked as designed, and its "nothing enforces it yet" clause still stands — this rebuild happened because a checkpoint needed the wheel inside the image, not because anything scheduled it. ~~`waiting-on: the next release that changes what an image carries — the A-28 rebuild duty (gsj-polar rides every release; nothing schedules it)`~~ (occurred at CP-85: 0.1.8, index `30e9d940…` — the cell that follows and the CP-85 report; struck at CP-98) **[CP-85] The 0.1.8 event occurred.** After PyPI publication, demo `3286271` raises its floor and image to 0.1.8. `gsj-polar:f0e8343a-gsj0.1.8` index `30e9d940…` publishes amd64 `b6a74e33…` and arm64 `7f5898b7…`; anonymous index/child reads and container import/payload proofs pass for both. Both report release source `effd658` and estate payload `ddbb661c…`. Fresh-clone arm64 up/preflight/row-2 submit/read/export/down passed with the unchanged read-only `.env` mount and no exported submit token. The core submit implementation stayed byte-identical, but the image carries the changed estate/pipeline payload; the report’s phase-3 obligation and A-28 duty require the recut. ~~`waiting-on: the next published library release that changes the image payload — recut and prove both platforms plus the consumer again`~~ **[CP-91] The 0.1.9 event occurred after publication.** Demo `530a3bc` moves its floor/tuple and image to 0.1.9, generates decisions inside the corpus, selects the corpus default and retains the external fallback. `gsj-polar:f0e8343a-gsj0.1.9` index `78d69417…` has amd64 `77c3c09f…` and arm64 `fc075348…`; anonymous registry reads, installed version/source/payload/harness imports and mounted-.env actual submit-byte proofs pass on both platforms. Both carry release source `4ac4e3c`, estate `75dfa5ce…`, pipeline `a291ec56…`. The original recipe's first build failed on pip network downloads; its successful retry required no product change. A fresh clone with the plain published wheel generated/validated the inside drop, created its previously missing work/runs directory, reached up/preflight, accepted and independently validated a CPU-only row-2 episode, read/exported it and stopped all five owned containers/network. All original IDs restored. MCP/harness image payloads are unchanged. ~~`waiting-on: the next published library release that changes an image payload — repeat the two-platform recut and fresh consumer proof`~~ (occurred at CP-93: 0.1.10 changed the estate payload, the cell that follows; struck at CP-98) **[CP-93] 0.1.10 delivery obligation:** the estate payload changes, so the demo must re-cut `gsj-polar:f0e8343a-gsj0.1.10` from public `v0.1.10` and PyPI 0.1.10, only after publication. Completion requires anonymous pulls of both linux/amd64 and linux/arm64 children, each reporting the new wheel, plus a fresh-clone accepted/read-back CPU episode. The printed CP-93 report holds the measured index/child digests, demo commit and outcome. MCP 0.5.0 and the harness payload are unchanged. ~~`waiting-on: the CP-93 post-publication two-platform image and fresh-clone episode proof completing — after completion, the next published library release changing an image payload repeats this duty`~~ (fulfilled at CP-93 — its report's "A-28 fulfilled" and "Actual fresh public clone" sections: index `ed35bc96…`, amd64 `a0891339…`, arm64 `f79c1054…`, both pulled with an empty auth config and executed, each reporting 0.1.10 and release source `f2537e7`; the fresh public clone's accepted episode `sk-polar-91d3cc49…` on that image; struck at CP-98) **[CP-95] 0.1.11 delivery obligation — the token's event occurs here:** the estate payload changes (CP-94's `estate.py` rides the wheel), so the demo re-cuts `gsj-polar:f0e8343a-gsj0.1.11` from public `v0.1.11` and PyPI 0.1.11, only after publication. Completion requires anonymous pulls of both linux/amd64 and linux/arm64 children, each reporting 0.1.11 and the release commit, plus a fresh-clone accepted episode read back on that image. The printed CP-95 report holds the digests, the demo commit and the outcome. MCP 0.5.0 and the harness payload are unchanged. ~~`waiting-on: the CP-95 post-publication two-platform image and fresh-clone episode proof completing — after completion, the next published library release changing an image payload repeats this duty`~~ (fulfilled at CP-95: index `40cb3484…`, demo `7035f12`, the CP-95 report) **[CP-97] 0.1.12 delivery obligation — the token's event occurs here:** the estate payload changes again (CP-96's probe class, clocks, headline and heartbeat, plus CP-97's pins skeleton ride the wheel), so the demo re-cuts `gsj-polar:f0e8343a-gsj0.1.12` from public `v0.1.12` and PyPI 0.1.12, only after publication. Completion requires anonymous pulls of both linux/amd64 and linux/arm64 children, each reporting 0.1.12 and the release commit, plus a fresh-clone accepted episode read back on that image. The printed CP-97 report holds the digests, the demo commit and the outcome. MCP 0.5.0 and the harness payload are unchanged. ~~`waiting-on: the CP-97 post-publication two-platform image and fresh-clone episode proof completing — after completion, the next published library release changing an image payload repeats this duty`~~ (fulfilled at CP-97 — its report's "The demo, after publication" section: index `f9f2e217…`, amd64 `bb73342e…`, arm64 `17390699…`, both children read and pulled anonymously, each reporting 0.1.12 and `3ac3fcd`; the fresh clone's `sk-polar-7922d33b…` on that image; struck at CP-98) **[CP-98] The register pass.** Every token above whose event had occurred is struck on its report's evidence AND a live anonymous read of each index on 2026-09-08 (a bearer from GHCR's token endpoint, `Accept` the OCI index): the harness `pi0.83.0-3` → `1fc083d4…`, the retrieval service `0.5.0` → `17704c06…`, `gsj-polar` `gsj0.1.8` → `30e9d940…`, `gsj0.1.10` → `ed35bc96…`, `gsj0.1.11` → `40cb3484…`, `gsj0.1.12` → `f9f2e217…` — all six `linux/amd64` + `linux/arm64`. The duty itself stands, unchanged in shape: ~~`waiting-on: the next published library release that changes an image payload — the two-platform recut and the fresh-clone episode on it, after publication (the CP-93/95/97 shape)`~~ **[CP-100] 0.1.13 delivery obligation — the token's event occurs here:** the packaged estate payload changes again (CP-98's reaper and CP-99's gateway probe, the six sites in `estate.py` where a measurement's word stood over an inference (five user-visible, the sixth a source comment) together with the probe's own "and from this host" leg, inferred since CP-03 and dialled now, the re-worded `vfs` warning and the conditional pins skeleton all ride the wheel), so the demo re-cuts `gsj-polar:f0e8343a-gsj0.1.13` from public `v0.1.13` and PyPI 0.1.13, only after publication. Completion requires anonymous pulls of both `linux/amd64` and `linux/arm64` children, each reporting 0.1.13 and the release commit, the pulled image's packaged `estate.py` sha256-equal to the published wheel's, plus a fresh-clone accepted episode read back on that image. This is also the cut that ends the state CP-99 left behind, in which a demo-door reader runs the demo's own fixed `bootstrap.py` over the library's unfixed `estate.py` inside one image. The printed CP-100 report holds the digests, the demo commit and the outcome. MCP 0.5.0 and the harness payload are unchanged, and are not recut. ~~`waiting-on: the CP-100 post-publication two-platform image and fresh-clone episode proof completing — after completion, the next published library release changing an image payload repeats this duty`~~ (fulfilled at CP-100 — its report's §3: index `f4a4cb2f…`, amd64 `cac43222…`, arm64 `17abd169…`, both children read and pulled anonymously, each reporting 0.1.13 and `91f99fd`, the packaged `estate.py` `e3d3d8d6…` equal to the published wheel's; the fresh clone's `sk-polar-0fd244cb…` on that image; struck at CP-102) **[CP-102] 0.1.14 delivery obligation — the token's event occurs again:** the packaged estate payload changes (CP-101's `skeleton_footer_row` — the conditional `== run <name> ==` footer — rides the wheel: `estate.py` sha256 `c77d4f2f…` against 0.1.13's `e3d3d8d6…`, while `gsj_rollout/` moves by the version literal only), so the demo re-cuts `gsj-polar:f0e8343a-gsj0.1.14` from public `v0.1.14` and PyPI 0.1.14, only after publication. Completion requires anonymous pulls of both `linux/amd64` and `linux/arm64` children, each reporting 0.1.14 and the release commit, the pulled image's packaged `estate.py` sha256-equal to the published wheel's, a fresh-clone accepted episode read back on that image — and, this time, the walkthrough's step 1b pair run on it. MCP 0.5.0 and the harness payload are unchanged, and are not recut. The printed CP-102 report and this paragraph's record clause hold the digests, the demo commit and the outcome. ~~`waiting-on: the CP-102 post-publication two-platform image and fresh-clone episode proof completing — after completion, the next published library release changing an image payload repeats this duty`~~ (fulfilled at CP-102 — this paragraph's record clause and the printed report: index `9526f3f7…`, amd64 `a7a498ab…`, arm64 `dab8d7bf…`, both children read and pulled anonymously, each reporting 0.1.14 and `8fb7207`, the packaged `estate.py` `c77d4f2f…` equal to the published wheel's; the fresh clone's `sk-polar-5f6705d8-ed…` and step 1b's `sk-polar-e2a5f0f7-a7…` on that image; struck at CP-102 — the next published library release changing an image payload repeats this duty) **[CP-105] 0.1.15 delivery obligation — the token's event occurs again, and ADR-0043 orders it:** the packaged estate payload changes (CP-104's round-seven `estate.py` rides the wheel: sha256 `e6598eeb…` against 0.1.14's `c77d4f2f…`, while `gsj_rollout/` moves by the version literal only), and since CP-104 `up`'s closing block derives this image's tag from the installed version, so the printed reference is only as good as the cut. The demo re-cuts `gsj-polar:f0e8343a-gsj0.1.15` from public `v0.1.15` and PyPI 0.1.15, only after publication — and before `main` moves: the release commit stays on a branch (CI on its pull request, the rehearsal dispatched on it) until the image pulls anonymously, so `main`, the guide pages and the page `up`'s NOTE links to never name a tag that does not resolve. Completion requires anonymous pulls of both `linux/amd64` and `linux/arm64` children, each reporting 0.1.15 and the release commit, the pulled image's packaged `estate.py` sha256-equal to the published wheel's and carrying CP-104's seams, the window between PyPI publication and the first anonymous pull measured, and a fresh-clone accepted episode read back on that image with step 1b run on it. MCP 0.5.0 and the harness payload are unchanged, and are not recut. The printed CP-105 report and this paragraph's record clause hold the digests, the window, the demo commit and the outcome. ~~`waiting-on: the CP-105 post-publication two-platform image, main moved after it, and the fresh-clone episode proof completing — after completion, the next published library release changing an image payload repeats this duty`~~ (fulfilled at CP-105 — index `24cbe2ea…`, amd64 `cb8786d4…`, arm64 `78609290…`, both children read and pulled anonymously, each reporting 0.1.15 and `d3f7120`, the packaged `estate.py` `e6598eeb…` equal to the published wheel's, CP-104's seams inside; the window PyPI 18:37:45Z → the first anonymous 200 at 18:53:08Z, 15 min 23 s, `main` moved at 18:55:18Z; the demo's fresh clone of `34d9d8c` on that image, step 1b's `sk-polar-09c4189e…` (t=4, page 4 cited) and the t=2 `sk-polar-981ed6b6…` (pages 1–2 only); struck at CP-105 — the next published library release changing an image payload repeats this duty) **[CP-106] 0.1.16 delivery obligation:** the accepted estate-phase cleanup changes both packaged tooling modules without changing their behavior or source homes. Re-cut `gsj-polar:f0e8343a-gsj0.1.16` from public `v0.1.16` and its actual PyPI wheel for linux/amd64 and linux/arm64. ADR-0043 still holds main/front-door advancement behind anonymous manifest reads, both child pulls and execution proofs of version, source, payloads, inheritance and behavioral seams. The existing demo must pass its full suite and fresh-clone accepted/read-back proof, including step 1b at t=4 and its t=2 counterpart, before its tested release changes are pushed and completion is recorded. MCP 0.5.0, Forgejo, pi, Polar and its three patches are unchanged. ~~`waiting-on: CP-106 publication, matching two-platform image proofs, main advancement after those proofs, and tested demo release push completing`~~ (fulfilled at CP-106 on 2026-09-11: both indexes serve the release wheel `7eba96f1…`; Polar index `1ae10e8c…`, amd64 `68bdc65f…`, arm64 `d5942f97…`, both anonymous pulls and execution proofs pass before main advances to `3b25931`; demo `0629cda` passes 89 tests and the fresh-clone t=4/t=2 cutoff, read-back and independent-validation proof, then pushes at 03:22:22.926703Z; t=2 answer quality is qualified in the completion entry below). `waiting-on: the next published library release changing an image payload repeats A-28 and ADR-0043 delivery and proof duties` |
...
the next fresh id in both consumer registers.

Release commit `4ac4e3c` is tagged `v0.1.9`; the record follows it. CI
[34052152185](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34052152185)
is GREEN in all four jobs: root 176, corpus 361 plus the existing local-data
skip, MCP 164, wheel install proof. Rehearsal
[34052154207](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34052154207)
is build GREEN, TestPyPI GREEN, PyPI SKIPPED. Tag release
[34052612922](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34052612922)
is build GREEN, TestPyPI GREEN, PyPI GREEN — three release jobs, not four.
Local, rehearsal and tag artifacts plus both public downloads are identical:
wheel SHA256 `cd734e7d9c6a1b9ada03e67498f066b1b6c722342754f1eccc7ffc0c5e711c13`.
It still has 18 entries; CP-88 changed existing force-included modules.
Nine excluded path components absent, seven required entries present, twine
PASSED; installed estate `75dfa5ce…` and pipeline `a291ec56…` match source.

Both fresh public-index installations pass all seventeen checks from a cwd
outside every repository, with imports asserted under site-packages. PyPI
uses plain `pip install gsj-harness-rollout-server==0.1.9`, no index flags.
The fourteen inherited checks retain their limits: fixture provenance and
synthetic submit bytes are not an accepted episode or a quality claim.
Check 15: owned `fred` under a 0500 runs root refuses cleanly, naming that
root, with zero Docker calls. All 23 installed CP-90 seam regressions pass.
Check 16: the legacy stored line refuses both replacement channels before
any secret read; hand-editing the printed `KEY='value'` form, then real
`up`, re-adopts the exported credential, persists it and passes verify.
Check 17: wheel scaffold → put thirty XML files in `decisions/` → validate
→ up writes the lock, mounts the corpus drop by default, and verifies the
tree/run/served agreement: 30 files, 313 units, drop `b414c670…`, 317 pieces.
Validate itself is read-only; up's pipeline scaffold phase writes the lock.
A second populated drop refuses; checked down and independent container/
network absence complete each proof. Engine metadata/tokenization uses a
scratch Qwen3-0.6B adapter with every parameter float32 on CPU; no inference
or request is sent to the standing Metal engine.

Consumers can carry decisions with the corpus, follow a working legacy
credential cure and receive a named refusal from an unwritable runs root.
The frozen estate README's flag-only and phase-3-pending wording remains
on rows 73/81, along with its omitted decisions lock. Row 81 now also names
review R-07's frozen core reader and test wording about the retired
apostrophe encoding. Row 80's workflow prerequisite remains open. No reward
redesign or B18 cure ships; the coming container stranger runs inherit this
phase-3 delivery, including corpus-contract v3 and CP-90's estate seam.
A-28's demo delivery is complete in `530a3bc`, committed and pushed only
after PyPI publication and a successful fresh-clone proof. Its tuple and
remedies floor at 0.1.9; the generator writes `<out>/decisions/`, bootstrap
selects the corpus route by clearing recorded external overrides, and the
sibling directory remains the fallback. Two populated drops refuse before
image work. The existing owned-directory setup now also creates `work/runs`,
which the new published mutation boundary requires on a fresh clone.

The image recut was required because the packaged estate/pipeline payload
changed. Published index `78d69417…`, amd64 `77c3c09f…`, arm64 `fc075348…`:
anonymous reads, exact release/payload checks, harness imports and actual
CLI submit with a mounted synthetic `.env` pass on both. The unchanged
recipe succeeded on retry after external pip/clone/upload stalls; no image
or product workaround was introduced. MCP/harness image payloads remain
unchanged. Existing demo suite 28 passes; route/default/conflict, quoted
inward migration and deterministic generator checks pass in scratch.

Fresh-clone up 42.248 s, preflight 9.213 s, row-2 submit 16.183 s,
read/export/independent validation all succeed; down 4.718 s restores the
original three container IDs and leaves no owned network. Accepted episode
`sk-polar-f8d6b205-2645-4f68-92e9-f7dce552f843`: orchard@2, one turn, zero
actual tools, 23 trainable tokens, no deliverable, findings `[]`. The CPU
adapter has no tool parser: literal tool-call text and preflight's parser/
sampling warnings remain explicit; this is provenance, not answer quality.
Both actual generations used CPU; the owned engine was stopped, port 18990
closed. No H200 or GPU workload occurred.

Rows 64/78 reconfirm the installed credential/descriptor seam; row 82 is
closed for consumers with both public proofs and the image proof; row 73's
contract and demo delivery events occurred, with frozen README/drop-only
update residues retained. A-28 re-parks on the next payload-changing release.
Library release and following record are pushed; demo `530a3bc` is pushed;
examples `04fc0e8` is unchanged, push up to date; archived predecessor
...
**[CP-93] Release 0.1.10 — the stranger fixes reach consumers.**

This patch release carries CP-92’s `estate.py` unchanged: the sandbox
refusal prescribes a plain native-platform pull first; download failures
retain registry advice while extract/mount failures name the daemon’s
storage and `docker run --rm alpine true`; partial `status` reports the
phases that stand and includes the recorded corpus root in its resume
cure; four warnings link the published bring-your-own model/pins page;
and top-level help lists all seven verbs. The page remains published
documentation, excluded from the wheel, with its URLs carried by the wheel.
A stranger at 0.1.9 had none of those installed diagnostics. Core consumer
objects, pins, the corpus format, complete-run status and exit codes retain
their existing contracts. Scripts scraping diagnostic prose or depending
on Compose output arriving during the subprocess must adapt; that text
and delivery timing are not stable machine interfaces. These bounded
repairs warrant 0.1.10 rather than a new minor contract.

The release’s acceptance gates are the eighteen-entry artifact with all
seven required paths and no banned prefixes; the unchanged three suites
(176 / 390 / 164); all four CI jobs; a real `workflow_dispatch` TestPyPI
rehearsal before the immutable `v0.1.10` tag; both public-index installs
outside every checkout with the standing seventeen checks and CP-92’s
new refusal/status/help/URL checks; and an accepted CPU episode read back
and independently validated from the installed library. Wishlist 84/85
close for consumers when those public proofs pass; row 60 stays closed.
The printed, untracked CP-93 report is the execution record, including
failures if a gate cannot be met. Stable public artifacts are the
[PyPI release](https://pypi.org/project/gsj-harness-rollout-server/0.1.10/),
[TestPyPI release](https://test.pypi.org/project/gsj-harness-rollout-server/0.1.10/)
and [release tag](https://github.com/MHGanainy/gsj-harness-rollout-server/tree/v0.1.10).
The operator’s explicit freeze exception moves only the package version
literal and its scaffold-test pin together to 0.1.10, so run.json provenance
agrees with the distribution. No line count or size equality moves.
This checkpoint’s explicit one-commit-per-touched-repository instruction
supersedes the usual release/record two-commit convention; the publication
tag is never moved to add later run IDs.

Measured echoes: CI’s corpus comment and display name now say 390; its
size comment says 2,034; README and CLAUDE count the actual 40 ADRs through
0040 and describe the collected suites. Row 53’s residue is closed and
row 79 closes on green Actions with those counts. ADR-0040 declines the
NOTE allowance: the wheel already explains that vendor is absent, the
unbuilt-checkout recipe has a published documentation cure, and the
measured two-line correction plus assertions/equality is deferred to the
next Polar re-vendor’s recipe rewrite. Row 86’s other frozen and declined
items retain their individual dispositions; there is no blanket close.

After PyPI publication, the demo’s single commit owns its >=0.1.10 floor,
F-85(c)’s library-flag attribution and F-86’s Docker-run prerequisite and
split pull-failure text. A-28 requires the re-cut two-platform
`gsj-polar:f0e8343a-gsj0.1.10`, anonymous children, version/payload checks
and a fresh-clone accepted episode read back on that image. The report
states whether that consumer delivery completed and the push outcome for
all four repositories. No F-id is minted: the retained examples trailer
is F-83, the demo’s next is F-87. The examples and archived predecessor
remain untouched. No H200, GPU, reward, B18 cure or later-phase work is
part of this release.

**[CP-94] What round three found — four strangers, both doors, four accepted episodes, and what they found anyway.**

Round three ran on 2026-09-07: four containerised strangers (two codex, two claude; two on the demo's README, two on the library from PyPI at 0.1.10 — the release CP-93 published to fix round two), a foreign `qwen3.6-27b` engine nobody here operates, and every one of them reached an accepted episode — the demo door, which never once passed `up` in round two, passed twice. The pins walk reproduced independently twice: one library stranger on the scaffold derived `G2: 8b159eb8…` (CP-92's value, round two's value); another wrote its own six-page corpus, got a different G2 correctly, ran the skill row, and landed on the identical tail `[248045, 74455, 198, 248068, 271, 248069, 271]` and eot `248046`. `bring-your-own.md` works. What they found anyway, by what it was (wishlist 87–93, demo F-87–F-92): **pin coverage** — a foreign-endpoint estate's `pins.gsj.json` had two of eight slots empty (G4, correctly), G3/G7's sets carried from the reference, and `provenance.engine` describing the reference machine, and its accepted episode read exactly like one on the reference stack (the per-gate table on the page and in the demo README; `coverage` blocks in both pins writers; the demo's `provenance.engine` re-recorded for the endpoint that serves, the reference block kept as `carried_from`; `up`'s pins line names the empty sets; the third question — submit or read time — argued and priced in ADR-0041: the pins file and the estate side, the demo's `read.py` parked on its next format bump, never `gsj_rollout/`); **`status` on a run that has not died** — three strangers, three wrong answers (a live `up` declared dead; a run whose services outran its record read as `mcp not reached` above `Up About a minute`; the demo's `status` mid-pull suggesting a second `up`): the run lock is the discriminator and `status` has three states — ACTIVE (say wait, suggest nothing), incomplete corrected against the daemon, complete; the demo's `up`/`down` hold a lock of their own; **the sandbox image checked last** — 41 minutes of pipeline before a refusal for an image whose name was printed at `+0.0s`: the check moved to the preflight before any container is pulled or created, the refusal text unchanged; **the engine probe the page did not have** — the tool-call parser's presence is testable read-only, `probe_model.py` step 5 asks (HTTP 400 quoted); **the borrowed endpoint** — "pin them server-side" assumes you serve the model; the page and the demo README write the other case down (provenance work, not training-distribution work) and the pins file records `sampling_policy: UNKNOWN` where a trainer sees it; **the pull that is healthy and silent** — 21 and 39 minutes without a line: a heartbeat once a minute with elapsed time and this host's received bytes, in both pull sites, and the three checks before concluding a pull hung; **the demo's six** (the printout's `--row 0` against the walkthrough's `--row 2`, the decisions surface never exercised, "two retrieval tools" against an eleven-tool roster, which file `preflight` reads after `up`, the empty `work/sessions/`, the "skips saying so" comma) plus the under-sold `page_census.pages_beyond_timestep`. The strangers' own errors — an `EXIT=0` that was `tail`'s, a G5 regex on raw bytes, a prerequisite read then skipped, `python` for `python3` — were flagged by their authors and are not findings; slow pulls are a pipe, not a regression; the page's published `248046` for the endpoint under test is good documentation and a contaminated experiment, and the fix is a different endpoint. Nothing in `gsj_rollout/`, `vendor/`, `pins/` or `tests/` moved; the census stays 2,034; delivery of `estate.py`'s changes waits for the next published wheel (row 88's token). Pushed: the library and the demo; the examples and the archived predecessor untouched.

**[CP-95] Release 0.1.11 — the round-three fixes reach consumers.**

This patch release carries CP-94's `estate.py` byte-unchanged and nothing else of substance: `status` in three states (ACTIVE while a command holds the run's lock — exit 0, the phases that stand, "wait"; incomplete with the retrieval-service row corrected against the daemon — exit 1 and the CP-92 resume cure; complete — the standing report), the sandbox image checked before any container is pulled or created (a stopped daemon named as such; `docker info` and `docker image inspect` the only Docker calls before the refusal, the refusal text unchanged), the pull heartbeat (docker's layer lines live, and every 60 s — `GSJ_ESTATE_PULL_HEARTBEAT_S` — elapsed time and this host's received bytes, labelled a pipe signal, never the pull's own count), `up`'s pins line naming the approved sets left EMPTY, help on four `up` flags (three bare at 0.1.10 — `--end-of-turn-token-id`, `--context-window`, `--max-tokens` — and `--gateway-host`'s one line extended with its probe default) and `--runs-dir`, and the wheel-aware closing block. A stranger at 0.1.10 had none of them: round three found every one against that wheel. Why a patch: `gsj_rollout/`'s eight modules are byte-identical to 0.1.10 except the version literal (checks, client, config, the receiver, the wire, the pins and the corpus contract unchanged; the same 18 entries); what moved is the estate tool's text, its stdout rows, the order of one check and a heartbeat. The one shape a script could have leaned on: `status` on a run whose lock is held now exits 0 with `(ACTIVE — …)` in its first line where CP-92's code printed `incomplete` and exited 1 with a resume cure — a wrong answer on a live run — the shape one library stranger and one demo stranger hit; the third `status` finding, a run whose services had outrun its record, is the daemon correction's; the incomplete path keeps exit 1 and its cure, the complete path keeps exit 0 and its first line, and a complete run under a live `update` or `down` reads `(ACTIVE — …)` too, exit 0, its standing first line back once the lock drops; run.json stays a schema-1 record with one additive key — the `pins` block gains `empty_sets`, which no reader of ours consumes. A script that read `status`'s exit 1 as "the run died" must now read the first line as well — priced and accepted as a patch: the old exit 1 on a live run was a defect, not a contract, and no consumer of ours polls `status` from a script (the demo's `bootstrap.py status` prints its own report).

The acceptance gates, as at CP-93: the eighteen-entry artifact with the seven required paths and no banned prefix (asserted locally on the built wheel before the push); the three suites unchanged (176 / 411 / 164 — the CI corpus label 411 verified against the runner's own collection, 410 passed + the local-data skip); all four CI jobs; a real `workflow_dispatch` TestPyPI rehearsal before the immutable `v0.1.11` tag; both public-index installs outside every checkout — CP-93's seventeen standing checks, CP-92's 28 transplanted regressions and four warning URLs, and, new for this release and driven from the installed artifact: `status` in all three states on a real installed `up`, the sandbox refusal before any container is pulled or created (`docker info` and `docker image inspect` its only Docker calls) with `docker ps -a` identical before and after and no `run.json`, the heartbeat on a pull slow enough to beat, the empty-sets line under a `GSJ_PINS_PATH` naming a foreign estate's pins with G4 empty, `up --help`'s five texts, CP-94's 21 regressions transplanted onto the installed module, and one accepted episode from the installed receiver and client read back — the §7 standard. Rows 88, 89 and 92 close for consumers when those public proofs pass; the printed, untracked CP-95 report is the execution record, failures included. Stable public artifacts: the [PyPI release](https://pypi.org/project/gsj-harness-rollout-server/0.1.11/), the [TestPyPI release](https://test.pypi.org/project/gsj-harness-rollout-server/0.1.11/) and the [release tag](https://github.com/MHGanainy/gsj-harness-rollout-server/tree/v0.1.11). The operator's freeze exception moves the package version literal, `__init__.py`'s and its scaffold-test pin together (the constant `script_version()` stamps into every run.json), no line-count or equality change; the one-commit-per-repository instruction governs again, so the tag is never moved to carry run ids.

Measured echoes, each against the tree: the shelf holds 41 ADRs (`README.md:112`; `docs/README.md:37`'s range 0027–0041 — the CP-93 sweep left it at 0038; CLAUDE.md's 0001–0041 landed at CP-94); the guide pages' wheel literal (`docs/guide/README.md:23`, `trainer-guide.md:9`); the stories a literal sweep cannot see, found by the pre-push review: `server-guide.md:224`'s "(a checkout; the next release)" on the three-state `status` → wheels from 0.1.11; the deprecated pipeline entry "retained in 0.1.9" → through 0.1.11 (`server-guide.md:308`, `trainer-guide.md:13`/`:20`, `corpus-contract.md:508`); `trainer-guide.md:11`'s wheel-layout range and `:22`'s per-wheel history of `estate.py`, which stopped at 0.1.9 and now names what 0.1.10 and 0.1.11 carry; `CLAUDE.md:85`'s corpus-suite count (390 → 411, the echo CP-93 and CP-94 both missed); CI's labels already true (176/411/164), unchanged — wishlist 79 CLOSED, its token's event having occurred at CP-93. Not lifted, recorded: `estate/README.md:91`'s retrieval-default history stops at 0.1.10 (0.1.11 defaults to 0.5.0 as well) and `:73`'s phase order still shows the sandbox check after the engine probe — both ride row 89/93's H200 bring-up token; the examples trailer's F-83 (row 86 (e)).

After PyPI publication, and only then, the demo's single commit moves its floor to `>=0.1.11` and its image pin; A-28's rebuild duty fires (the packaged estate payload changed): `gsj-polar:f0e8343a-gsj0.1.11` re-cut two-platform at public `v0.1.11` / PyPI 0.1.11, both children anonymous-pullable and reporting 0.1.11, and a fresh-clone run to an accepted episode on that image. Demo F-87 and F-92 stay parked on `read.py`'s export format bump (`gsj-demo-episode-export/3`), which this release does not make. No F-id is minted (the examples trailer's F-83 stays; the demo's next fresh is F-93). The examples repo and the archived predecessor are untouched. No H200, no GPU, no reward, no B18, no phase 6.

What a fourth round inherits: a `pip install` now carries `status`'s three states, the sandbox-first refusal, the heartbeat, the empty-sets line, the help text and the wheel-aware closing block; the demo's pinned image carries the same module. The page's published `248046`/tail still names the endpoint round three used — give the next round a different one (row 93).

**[CP-96] What round four found — four strangers, six accepted episodes, and an `up` that never exited 0 on either A-door container.**

Round four ran overnight 2026-09-07/08: four containerised strangers (two codex, two claude; two on the demo's README, two on the library from PyPI at 0.1.11 — the release CP-95 published to fix round three), the same borrowed `qwen3.6-27b` engine (the operator accepted that the model-derivation half was uncontrolled: the page's published `248046` still names it, and a fourth stranger measured the values before cloning so its own walk stayed clean), nested Docker on `vfs`, and a pre-pull diagonal (the harness image seeded in a1 and b2, absent in b1 and a2). All four reached an accepted episode — six in all — and `bootstrap.py up` never once exited 0 in either A-door container across four attempts. The pattern is the finding: CP-94 instrumented the phases it had measured, and every long silence and every hard failure this round was in a phase it had not — container create, `compose up`, the decisions embed, and the two readiness probes. The treatment was right and had been applied to a list, not to a class; this checkpoint generalised it (wishlist 94–99, demo F-93–F-100, ADR-0042).

The readiness probes (row 94, demo F-93): both codebases probed by `docker run --rm` of a heavyweight image under a hard timeout, as the last step, and on a copy-on-create daemon the create alone outlasts the budget — four bare `subprocess.TimeoutExpired` tracebacks in three containers with the estate healthy underneath, a `Created` container leaked each time. Fixed as the class: the dial runs by `docker exec` inside a container the run already has (nothing created, nothing leaked), the fallback is named and removed in a `finally`, a probe that cannot run degrades naming `--gateway-host`, the candidates and the resume, `--skip-sandbox-image`'s second effect is gone, the fallback label names its origin, the help says how to choose, and both daemon checks price `vfs` at their first Docker call. The clocks (row 95): a2's "9 h 22 m embed" and "timeout off by 19×" were measured across the operator's laptop sleeping — withdrawn as measurements; the source facts stand and were fixed: both readiness deadlines monotonic, the wall clock read beside them and a skew printed as the host having slept, every refusal printing what it waited beside what it budgeted, a heartbeat while nothing changes, `--ingest-timeout`'s two ends on one clock, the health read 15 s, the log tail rate-limited, the `--rebuild` re-run attaching instead of re-embedding from zero. The consumed budget read from a1's ready line: ~200 s of 1800 on that host (the `(1294.1s)` suffix is the phase, 1094 s of it the container's create) — comfortable there, marginal at the pessimistic end of a2's unmeasurable bound. The silent embed (row 96): the service's `build` block, published since CP-77 and read by nobody, is on the poll line now (`building decisions: batch i/N`); the demo README's "about 5 seconds" carries its hardware. The demo's `status` (F-94) reports a partial estate instead of dying on it; its seven (F-95–F-100): `--ingest-timeout` forwarded, the submit recipe on disk the moment it is derivable, preflight's contradicting cure suppressed and its exit codes stated, the storage driver read, and the README corrected against reality (three images plus Forgejo, pip's slow-pipe flags, `df -h` not `docker system df`, the driver's price, the embed figure labelled). The library's four: `verify`'s headline counts skips apart in the driver (row 97; the pipeline's own headline waits for its lift), `--skip-sandbox-image` and `--gateway-host` (in row 94), the 1800 s message (in row 95); the heartbeat names the layer phase in both codebases (row 98, F-99). Step 7: the pins skeleton decided and parked (ADR-0042, row 99); F-87's second independent arrival named its event, the next demo sitting that opens `read.py`. Recorded and dismissed: the strangers' own withdrawn errors (a Docker boundary misread, a 2-second diagnostic's duration on a 980-second submit, a recorder's `PYTHONPATH`, a snapshot of a long job's last minute), `python` vs `python3` for the second round running, 45–57-minute pulls of the retrieval image (observability, not speed), the README's citation under-promise (this round's 27B wrote twelve grounded `dec:` tokens — reworded as what those episodes did), and the leak, whose fix is a different endpoint in round five. Observed, not taken: Polar's 600 s sandbox-create budget (`docker create failed with exit code -1: None`, twice on the loaded `vfs` host; 49 s when it was quiet) — the storage-driver warning names it. Nothing in `gsj_rollout/`, `vendor/`, `pins/`, `tests/`, `estate/mcp-service/` or `ingest_corpus.py` moved; the census stays 2,034; delivery waits for the next published wheel. Pushed: the library and the demo; the examples and the archived predecessor untouched.

**[CP-97] The pins skeleton, and release 0.1.12 — round four's fixes reach consumers.**

The checkpoint carried one piece of source work before the cut, ADR-0042's parked implementation: `estate.py up` now writes `<run>/pins.skeleton.json` beside `rollout.yaml` (row 99) — the two carried sets from the pins in force, the G6 tail and the end-of-turn id measured from the endpoint's own render under the kwargs pi sends for the run's `--thinking` (the `bring-your-own.md#your-model` measurement performed by `up`; the demo's terminator rule for the closer; every request and response kept), the two derived sets and G4's empty, `not_measured` and `coverage` as the page's script writes them. Two guards keep it from being pins: its `format` is `gsj-pins-skeleton/1`, and a `GSJ_PINS_PATH` that names it is refused by `up`/`update` before anything runs and by the library on first use (`PinsConfigurationError` naming the empty key and the file — measured in a fresh process over the tracked fidelity body). The cure for b2's finding 4 rides with it: `builder.end_of_turn_token_id` takes the measured id unless `--end-of-turn-token-id` says otherwise (an explicit answer persists across re-runs; a disagreement is warned about; the record labels its source), so the reference model's `151645` stops being the default on an endpoint that is not it. The page's `derive_my_pins.py` reads the skeleton and still derives the two and asserts the carried ones. ADR-0042 as written survived contact with one amendment (the format label); the amendment is appended to the ADR, untracked. Corpus suite 432 → 463 (`test_estate_cp97_skeleton.py`, 31 tests: hermetic renders for three template families, the library's refusal in a subprocess, the page's script run from its own text). On the Mac reference engine the measurement reproduces both packaged tails and `<|im_end|>` exactly.

Then the release. 0.1.12 is a patch: `gsj_rollout/`'s eight modules are byte-identical to 0.1.11's but for `__init__.py`'s literal (the census stays 2,034); `checks`, `client`, `config`, the receiver, the wire, both pins sets, the G2 capture and the corpus contract are unchanged; the artifact is the same eighteen entries. What moved is the packaged `estate.py`: CP-96's probe class, clocks, headline and heartbeat, and CP-97's skeleton. Two shapes a script could have leaned on are named and priced: `up` on a run whose gateway-host probe cannot run now exits 0 with a WARNING and a written `rollout.yaml` where 0.1.11 exited 1 with a bare traceback one file short — the old exit was a defect, not a contract, and no consumer of ours read that traceback as a signal; and `run.json` gains additive keys (`harness.end_of_turn_token_id_source`, `engine.tail`, `pins.skeleton`) that no reader of ours consumes. A refusal's text now carries the measured wait, and `up` writes one more file — neither a shape anything parsed. No minor bump. The acceptance gates, as at CP-95: the eighteen-entry artifact with the seven required paths and no banned prefix (asserted locally on the built wheel before the push, sha256 `82a5f6d5…`); the three suites (176 / 463 / 164); CI green in all four jobs with the corpus label re-set to what the runner collects (462 passed + the demo-thirty skip = 463); `workflow_dispatch` rehearsed to TestPyPI before the tag; both public indexes installed by name from a scratch venv outside every checkout and proven — the standing checks, and for the first time each CP-96 fix driven from the installed artifact rather than the checkout (rows 94–98's tokens), the skeleton written and refused as pins, and one accepted episode read back. Measured echoes, each against the tree: the shelf holds 42 ADRs (`README.md:112`; `docs/README.md:37`'s range 0027–0042; `CLAUDE.md`'s 0001–0042 already right); the version literal on its fourteen lines across nine files (`pyproject.toml:7`/`:97`, `gsj_rollout/__init__.py:16`, `tests/test_scaffold.py:7`, `CLAUDE.md:5`/`:51`, `README.md:51`, `docs/guide/README.md:23`, `docs/guide/trainer-guide.md:9`/`:11`/`:13`/`:20`, `docs/guide/server-guide.md:312`, `docs/corpus-contract.md:508`); the suite counts in CI's labels and `CLAUDE.md:73`/`:85`/`:96` (the `:85` echo CP-93 and CP-94 both missed, found by the pre-push review again); the deprecated pipeline entry retained through 0.1.12 (`pyproject.toml`, `corpus-contract.md`, `trainer-guide.md`, `server-guide.md`).

After PyPI publication, and only then, the demo's single commit moves its floor to `>=0.1.12` and its image pin; A-28's rebuild duty fires (the packaged estate payload changed): `gsj-polar:f0e8343a-gsj0.1.12` re-cut two-platform from the public `v0.1.12` and PyPI 0.1.12, both children pulled anonymously and reporting 0.1.12, a fresh clone run to an accepted episode read back on that image; MCP 0.5.0 and the harness image unchanged, not recut. F-87 and F-92 stay parked on the export format bump: this is a floor bump and an image cut, not the demo sitting that opens `read.py` — CP-96's reading holds. Row 97's standalone half stays parked and says so: `ingest_corpus.py` is read-only at CP-97, and the 0.1.12 wheel's own `verify` headline still counts a skip as a pass. Not touched: the examples repo (its trailer's F-83 stands), the archived predecessor. Not lifted, recorded: `estate/README.md:73`/`:101` — its run-directory enumeration omits `pins.skeleton.json` and run.json's three additive keys (`harness.end_of_turn_token_id_source`, `engine.tail`, `pins.skeleton`); the file is outside this CP's lift and the server guide links to it (row 99 carries the residue). One finding the proof made against the release itself, recorded not fixed (row 100): the CP-96 reaper's 30 s bound never engages because the real CLI's `docker rm -f` on a missing name exits 0 — the hermetic test faked exit 1 — so a fallback probe killed mid-create on a copy-on-create daemon still leaves its (now named) container behind; the exec path, the named fallback's removal when it exists and the degrade are proven. What a stranger gets at 0.1.12 that they did not get at 0.1.11: an `up` that cannot die at its last step on a copy-on-create daemon (the dial by `docker exec`, a named and reaped fallback, a probe that cannot run degrading with the cure named), waits that count only the time they waited and refusals that print it, the collection the retrieval service is building on the poll line, a heartbeat that names the pull's phase, a `verify` headline that counts skips apart, a warning that prices `vfs` at the first Docker call, help that says how to choose a gateway host — and a run directory whose `pins.skeleton.json` carries the measured tail and end-of-turn id, so the pins walk starts from measurements and `builder.end_of_turn_token_id` is no longer the reference model's by default. Pushed: the library (release commit, tag), the demo after publication; the examples and the archived predecessor untouched.

**[CP-98] The reaper's bound, and a register pass — a small lift before round five.**

CP-97's release proof found that the reaper CP-96 wrote never engaged: the real Docker CLI answers `docker rm -f` on a name that does not exist with exit 0 and the message on stderr, the loop read exit 0 as removed, and the hermetic test had faked exit 1 — so the 30 s bound that test proved never ran on any daemon, and 0.1.12 shipped it. This checkpoint is the freeze-lift row 100 waited on. The cure is two lines: `reap_container` reads *removed* off stdout, where the CLI prints the name it removed, and treats every other answer as not produced yet until the bound — stdout rather than stderr because a message that stops matching fails loud that way (the loop runs to its bound and says so) and silent the other way; `docker inspect` first was not taken (a second call per tick, a second string to fake, and no better at telling *never existed* from *still being created*). The third state is the code path's: a `docker run` that raised `OSError` never ran a client, so nothing is reaped. The fake was corrected to the measured shape and shown catching the old code — against CP-97's `estate.py` in a detached worktree the corrected test fails on the call count and on the bound, and five of the new module's tests fail with it (6 failed / 22 passed there; 29 passed here). The bound was then measured engaging under the conditions CP-96 named: a nested dockerd 29.8.0 on `vfs` under six and then twelve concurrent creates of the 2.65 GB retrieval image, the fallback `docker run --name gsj-probe-<pid>` killed at 20 s mid-create — the reaper asked 16 times over 31 s (every answer exit 0 + `No such container`), gave up naming the container, the wait and the command, and the container turned up `Created` 35 s after the kill under six creates and 90 s under twelve, removed by the message's own cure; with the bound raised past the create the reaper waited and removed it (50 s and 223 s after the kill), its last answer's stdout naming the container.

The class, not only the row — **§8 rule 10, decided**: when a test fakes a CLI, the fake's exit code and streams are part of the contract, and something must measure the real one. Twice now a fake's shape diverged from the real thing and shipped green (CP-84's Compose grammar, row 100's `rm -f`), and a fake written from memory proves the author's model of the CLI, not the CLI. The mechanism: `estate/corpus/tests/cli_shapes.json` records every shape a test fakes and the code branches on — exit code, stdout, stderr, what reads it, the CLI and daemons it was measured on; the corpus suite re-measures the file against the CLI on PATH wherever a daemon answers (the workstation, the CI runner; skipped naming the reason where none does, the H200's shape); a fake takes its shape from the file through `conftest.cli_shape`. The rule's first catch was its author: the file's first draft wrote `docker inspect --format … <missing>` with an empty stdout, the CLI prints a bare newline there, and the measuring test refused the draft on both daemons before it was committed. The retrofit took the reaper's fakes, the `docker inspect` fakes, a `docker info` fake still answering the pre-CP-96 one-word format, and an `image inspect` fake with invented streams; the rest were censused and agree on every field the code reads. Corpus suite 463 → 471; CI's job label still reads 463 — `.github/` is not lifted, and the label moves with whatever CP next lifts it.

`estate/README.md`, lifted for row 99's residue, was read whole against the tree instead — by hand for the rows, then by a seven-reader review with two adversarial verifiers per finding (35 agents; 14 stale claims, 11 upheld by both verifiers, 3 contested and one of those taken after checking the code myself). Taken: the phase order (the sandbox check first since CP-94, the engine measurement since CP-97), `status`'s three states, the pull's heartbeat and split refusal, the engine row's measurement, the retrieval-default history through 0.1.12, the run directory with `pins.skeleton.json`, `decisions.lock.json` and `run.json`'s additive keys, the probe as CP-96 and CP-98 built it — and eleven claims no row had recorded: the ChromaDB backend credited to 0.4.0/CP-58 when 0.3.0/CP-15 introduced it, `mcp-down` needing the same two variables as `mcp-up` (compose interpolates `${VAR:?}` on `down` too), `health` requiring an `endpoint.env` it only reads when present, the front door's exit code (`bringup` hands off to argparse, which exits 2), `serve-updated.sh` credited with a tunnel and an `endpoint.env` it has never written, the runs root the tool has refused to create since CP-90, which command picks up a drop replaced in place (CP-88 inverted it), the whole-corpus re-embed that per-collection fingerprints ended at CP-79, why G4's two sets are empty (no API exposes those bytes — not "until an episode supplies them"), and the slime recipe's link into a directory the examples repo deleted at its own CP-69. That last one is the scan's real catch: §7's [CP-69] paragraph had recorded it as a prose residual worded "the next estate-lift CP re-points it" — rule 9 (c)'s rejected wording, in a paragraph rather than a row, carrying no `waiting-on:` token — so no scan could see it and it sat through twenty-nine checkpoints while the link rotted. Rule 9's mechanism works on what it can see; a prose park is invisible to it, and the cure is the rule as written (a park lives in a register row, tokened) applied to the paragraphs too. Two tokened parks did fire and were taken: row 73's [CP-91] residue (the run-file list without `decisions.lock.json`, whose token named an authorized estate README correction — this lift) and row 58's, already closed at CP-92 and re-verified closed here. The residue had accumulated across four checkpoints because its token — the H200's next bring-up — is a cadence nobody schedules, rule 9 (c)'s class in a different coat: a README sentence that derives from the tree needs no park at all (it is the echo class every lifting CP verifies), the sentences that describe the box's own state are dated in the file now, and the one fact only the box can supply — where the static `skopeo` binary came from — carries a token on row 93 naming an observable event (the next checkpoint that runs any command there). Row 89's H200 token is retired. **The register pass**, struck on evidence re-verified, not on memory: rows 84 and 85 on the CP-93 report's installed-artifact proofs (both indexes, the sandbox refusal, the download and extract branches, the killed `up` and `status`); rows 88, 89 and 92 on the CP-95 report's table (`status` in three states, the sandbox-first refusal in about a second, the heartbeat's beats); A-28's six fulfilled tokens (CP-51, CP-61, CP-81, CP-91, CP-93, CP-97) on the CP-61/64/85/93/97 reports and a live anonymous read of every index on 2026-09-08 — all six images two-platform at the digests the reports recorded; the duty's token re-stated unchanged. Nothing was struck that a report and a check did not discharge; nothing was left open that they did. No release: a consumer on 0.1.12 has the named container and the message, not the bound, until the next published wheel; the demo's own reaper carries the same defect, recorded on row 100 with a token on the demo's next commit. Not touched: `gsj_rollout/` (2,034/2,034), `vendor/`, `pins/`, `tests/`, `pyproject.toml`, `estate/mcp-service/`, `ingest_corpus.py`, `.github/`, the demo, the examples, the archived predecessor. What round five meets that round four did not: a fallback probe whose reaper waits for the daemon and names what it could not remove, a suite whose fakes are measured, and a recipe that says what a run directory holds.

**[CP-99] What round five found — four strangers, three accepted episodes, one run killed by a probe that claimed a measurement it had not made, and one lost to the harness.**

Round five (2026-09-08/09) crossed two doors against two agents against **two storage drivers**, so a driver finding could finally be told apart from a software finding, and ran against 0.1.12 — the release CP-97 published and CP-98 corrected in source. The demo door finished twice (a1 one accepted episode, a2 two; zero quarantines, zero rejections). The library door finished neither: b1 stood a whole estate, submitted, and hung at `0/1 sessions terminal` for 22 minutes because `up` had written `polar.gateway.public_url: http://host.docker.internal:8200` and printed **"(measured: dialable from a container on 'gsj-helix-net' and from this host)"** over an address that answered from neither side, while the compose gateway that did work sat in the same candidate list recorded `reachable: true`; b2 refused every Docker command on a boundary reading of the operator's launch note and stopped 14 steps in. Three defects in one function (wishlist row 101): a dial that accepted ANY HTTP answer, so a foreign listener passed it; `host.docker.internal` inserted at index 0 on nothing but resolving, so the working candidate lost on ordering; and a gateway port already in use meaning `pass  # probe it`, which measured somebody else's listener and still printed `measured:`. Plus the half of that label which was never a measurement at all — "and from this host" was inferred from "the host reaches its own interface IPs trivially", true of an IPv4 and false of every name, and is dialed now. **Why five rounds missed it: the demo door never runs this code** — door A hard-codes a container-name gateway (`derived_from: "--gateway-host (not probed)"` in both a-run records) while door B must find one address a host process and a sandbox container can both dial. The library door's hardest problem does not exist on the demo door, which is the strongest argument yet for keeping both.

b1's sentence is the finding's real name — **"worse than not probing, because 'measured' told me to trust it"** — and it is the third occurrence of one class in four checkpoints: CP-96's reaper (a bound that never ran), CP-98's fake (a shape nobody had measured), and now a label that overstated what was measured. Each was green. So the audit the row demanded was made: every `measured` / `UNMEASURED` / `derived_from` string in `estate.py` re-read against the code path that writes it — the engine probe, the pins in force, the tail measurement, `status`'s three states, the heartbeat, the run record. **Six sites wrote a measurement's word over an inference, and one flag is the root of three of them**: a measurement REUSED from the record (the endpoint down on a re-run) was stamped `source: measured`, its `coverage` row said "measured here", and the closing block said "the G6 tail and end-of-turn id measured" where only the tail was; the skeleton's engine block said "as probed at T" for an endpoint that never answered; and a candidate comment called a recorded answer a measured one. All six are corrected here. Four more are recorded and left, with reasons, in the report: two historical measurements generalised beyond what was sampled (a Forgejo tag line, a Forgejo major version), `status`'s partial path presenting a recorded engine value in probe tense, and the skeleton's `measured` JSON key, which also carries pi's assumed chat-template kwargs — its members are each labelled, and the key is a published field of a shipped format, so a rename is a format change rather than a wording fix (the page's script reads `format`, `mode`, `not_measured`, `pins` and `provenance`, and would survive one). Found while proving the fix, and fixed with it: `docker exec <c> python -c …` on an image without python exits **127 with the OCI error on STDOUT and an empty stderr**, so CP-96's `returncode != 0 and not proc.stdout.strip()` guard recorded no failure and `up` refused with "no host address is dialable" for a probe that had never dialed — rule 10's machinery catching a live defect the second time it was used, on the checkpoint that used it.

The driver diagonal answered its own question and corrected this project in the process (row 104). Reach was exact: three warnings per run to both `vfs` strangers, zero to both `overlay2` strangers. But CP-98 and round five's own control record said `vfs` cost nobody the round, and it did — **19.4 s of sandbox init against 1.1 s on the identical task, 17×** (Polar's `init_ms`, both bodies on case_orchard t=2; the 731 MB image copy is the only plausible tenant of the delta) — while the warning's own numbers ("~13 GB per container", "minutes per create") were inferred once from a loaded host and are wrong by an order of magnitude. Mechanism confirmed, magnitude not: both warnings now carry the controlled pair's measurements (**31 G against 8.4 G of data root for the same 4.061 GB of images**, reproduced at 7.5× on the other door) and keep round four's blown 600 s budget as what it was. The diagonal's other result is about reading, not storage: a1 read that warning four times, never ran `docker info`, filed nothing and paid 31 G without knowing, while b1 read it once, ran `docker info` before any project tool, and raised its own timeouts because of it.

Three documentation defects, each found by more than one stranger and each with numbers (row 103): `troubleshooting.md`'s "a healthy pull never prints `Retrying in N seconds`", falsified 3/3 one sentence before "Only then interrupt `up`", with two strangers' hands on Ctrl-C; `df -h`, blind at this project's bandwidth (+155 MB in 60 s invisible behind a flat `47G`) in the very sentence that retired `docker system df` for the same false negative; and pip's resolver timeout, which names a dependency rather than a network. The leak is wider than recorded and sits on **both** doors (row 105) — the demo README published the operator's endpoint's `248046` inside the paragraph telling you not to write it into `config.yaml`, and **zero of four strangers produced an uncontaminated derivation**; the door-A half is fixed here, the door-B half needs an endpoint this repository has never published, which is round six's. And the skeleton: **zero of four opened it** (row 102). Three were written and all three agree exactly, which cross-checks CP-97's measurement path and says nothing about use. The decision is argued in the row — it stays where it is, and its prose becomes conditional on the pins in force, because a1's question ("why does a demo that already derived usable pins get a file saying the walk is not done?") had a true answer the file was not giving. What round five could not test is that nobody reached the moment the skeleton is FOR.

Three limits are recorded as rows rather than as sentences here, because §8 rule 9 says a park that only lives in a paragraph is invisible to the scan: whether the gates are load-bearing (three accepted episodes, zero rejections, zero quarantines, and no documented way to trip one — a2: "my entire belief that they are load-bearing rests on a table in a JSON file describing itself"), whether the cutoff binds at retrieval or only at checkout (row 2 alone cannot distinguish them; a t=2/t=4 pair can), and whether door B can be completed from PyPI alone (it cannot, and has not for five rounds — the cure taken is the documentation's, with the `gsj-polar` image named for the first time; the packaging cure is refused on the record, since shipping Polar reverses ADR-0020 and depending on it is impossible). What is the harness's is left to the harness (row 109): the launch note's boundary sentence, read as forbidding daemon introspection by 2/2 Codex agents and 0/2 Claude agents with the drivers crossed inside each pair, cost b2 its entire run and cost a1 the driver finding it was four warnings from — recorded for round six, not edited here, because a checkpoint that quietly changes the instrument destroys the comparison it exists to protect. **What round six inherits: a 0.1.12 whose probe is corrected in source and not on the wire** — until the next release, an installed `up` still accepts a foreign listener's answer, still puts a resolvable name first, still probes a port it cannot bind, and still prints "~13 GB per container"; the demo's `bootstrap.py` is fixed at its own commit and its image is not re-cut. **Not touched**: `gsj_rollout/` (2,034/2,034, unchanged — the skeleton decision needed no library line), `vendor/`, `pins/`, `tests/`, `pyproject.toml`, `.github/`, `estate/corpus/ingest_corpus.py`, `estate/mcp-service/`, the examples repo (its trailer keeps F-83) and the archived predecessor, which was not read from. **Pushed**: the library and the demo, one commit each.

**[CP-100] Release 0.1.13 — round five's fixes reach consumers, and the demo's mixed state ends.**

0.1.13 is a patch, and the argument is the same one 0.1.12 needed. `gsj_rollout/`'s eight modules are byte-identical to 0.1.12's but for `__init__.py`'s literal (the census stays 2,034/2,034, headroom 0 by design); `checks`, `client`, `config`, the receiver and the wire are byte-identical, both pins sets and the G2 capture too, and the corpus contract's rules are untouched; the artifact is the same eighteen entries with the same seven required paths and no banned prefix. What moved is the packaged `estate.py`: CP-98's reaper, which reads *removed* off the CLI's stdout so its 30 s bound engages on a name the daemon has not created yet, and the whole of CP-99 — a gateway-host probe whose label now means what it says. **The shapes a consumer could have leaned on are named and priced.** `run.json`'s `gateway_host_probe` keeps its `candidate` and `reachable` keys and gains two, `container` and `host`, each carrying `our sentinel` / `a foreign listener` / `nothing` (`host` also `not dialed`); `reachable` survives as a key and *tightens* as a claim — it was "something answered the container leg", it is now "this run's own nonce came back on both legs", so an address 0.1.12 recorded `true` can be recorded `false` at 0.1.13 without anything on the host changing. `harness.end_of_turn_token_id_source` gains a fifth value, `reused from the record's earlier measurement`, where 0.1.12 wrote `measured` over a value it had carried rather than measured. And the behaviour change that is the whole point: **an `up` that chose `host.docker.internal` at 0.1.12 may choose a different address at 0.1.13 on the same host** — that is the fix, `polar.gateway.public_url` is derived from it, and anyone who pinned the old answer by copying it out of a run record will see the new one disagree. None of these is a documented consumer surface (the `derive_my_pins.py` on `bring-your-own.md` — the only reader of a written skeleton this project publishes, and it is on a page, not in the wheel — reads `format`, `mode`, `not_measured`, `pins` and `provenance`, none of which moved), the skeleton's `format` stays `gsj-pins-skeleton/1` because no key was renamed, and no minor bump. The acceptance gates, as at CP-95 and CP-97: the eighteen-entry artifact asserted locally on the built wheel before the push, with the packaged `estate.py` and `ingest_corpus.py` sha256-equal to their frozen sources; the three suites (176 / 483 / 164); CI green in all four jobs, with the corpus label re-set from 463 to what the runner actually collects (482 passed + the demo-thirty skip = 483 — `.github/` was frozen at CP-98 and CP-99 and the label had gone stale by twenty); `workflow_dispatch` rehearsed to TestPyPI before the tag; and both public indexes installed by name from a scratch venv outside every checkout, with **each CP-98/CP-99 fix driven from the installed artifact rather than the checkout** — b1's topology reproduced against the published wheel with the published 0.1.12 choosing the losing address on the same box, a foreign listener refused and recorded by name, a port that cannot be bound labelled UNMEASURED, the exec-127 branch reported as a failure, the `vfs` warning carrying the measured pair on a real `vfs` daemon, the conditional skeleton written on an estate whose pins already cover it, the reaper's bound engaging, and one accepted episode read back. Measured echoes, each against the tree and never copied from a register row: the shelf holds 42 ADRs (`ls docs/decisions`; ADR-0042 twice amended, none added at CP-98 or CP-99, so `CLAUDE.md`'s 0001–0042 and `docs/README.md:37`'s 0027–0042 stand); the version literal on its eighteen lines across eleven files, two of them the `gsj-polar` image tag (`README.md:20`, `docs/guide/bring-your-own.md:7`) and one a marker CP-99 wrote for exactly this release (`troubleshooting.md:107`'s "Since 0.1.12+CP-99" → "Since 0.1.13"), with `trainer-guide.md:22`'s per-wheel history gaining its 0.1.13 clause; the suite counts in CI's two labels and `CLAUDE.md:74`/`:86`/`:97` (three stale 481s, and the sentence that said the label still read 463 because `.github/` was not lifted); and the deprecated pipeline entry retained through 0.1.13.

**What 0.1.13 does not carry, said as plainly as what it does.** `status`'s partial path still prints a recorded engine value in probe tense where its complete path re-probes — CP-99 recorded it and did not fix it, and this cut is not a fix. The two historical measurements generalised past their sample stand as they were (the Forgejo mirror's "measured digest-equal" over one or two tags on one date, and "measured on Forgejo 16" over a 16.0.x pin). `ingest_corpus.py`'s standalone `verify` headline still counts a skip as a pass: the file is read-only at CP-100 as it was at CP-96 and CP-97, the wheel carries it byte-unchanged, and row 97's token is not discharged by publishing it. `gsj_rollout/cli.py`'s serve printout still names only a `<checkout>` where `estate.py`'s now names a public image — row 108's residue, frozen here, and the size law's headroom of zero is why. And three of round five's findings are not the kind a release can close at all (rows 105, 106, 107): the door-B half of the published-values leak needs an endpoint this repository has never published; whether the gates are load-bearing needs a rejection or a quarantine read by someone who did not write the gate, and this checkpoint's own proof adds a fourth acceptance instead; whether the temporal cutoff binds at retrieval or only at checkout needs a t=2/t=4 pair. Row 109 is the harness's and stays the operator's.

After PyPI publication, and only then, the demo's single commit moves its floor to `>=0.1.13`, its remedy strings, its `polar.Dockerfile` default and its two `docker run` recipes; A-28's rebuild duty fires because the packaged estate payload changed: `gsj-polar:f0e8343a-gsj0.1.13` re-cut two-platform from the public `v0.1.13` and PyPI 0.1.13, both children pulled anonymously and reporting 0.1.13, the pulled image's packaged `estate.py` sha256-equal to the wheel's, and a fresh clone run to an accepted episode read back on that image. **That cut is this release's own reason for existing**: CP-99 fixed the demo's `bootstrap.py` at `f917295` while its pinned image still carries 0.1.12's `estate.py`, so a demo-door reader today runs the demo's new code over the library's old code inside one image — a state nobody has yet been observed in, because it began at that commit and no round has run since, and one this cut ends. **And the ordering is stated rather than glossed**: `README.md:20` and `bring-your-own.md:7` name `…-gsj0.1.13` in the release commit, which is necessarily before the tag that publishes the wheel the image is built from, so at the `v0.1.13` tag itself those two sentences name an image that does not yet exist. That is the same window the version literal has always had here (`README.md:63` says 0.1.13 before PyPI does), and it is a window rather than a claim only because this checkpoint closes it: the printed CP-100 report carries the publication timestamp, the image build, both child digests and the minute each happened. If the cut had failed, those two lines would have been the release's own defect and would have had to move back. MCP 0.5.0 and the harness image are unchanged and not recut. F-87 and F-92 stay parked on the export format bump: a floor bump and an image cut are still not the demo sitting that opens `read.py`, and CP-96's and CP-97's reading holds unchanged. Row 100's demo token, whose event occurred at CP-99 and whose strike was missed there, is struck here on that evidence. Not touched: the examples repo (its trailer's F-83 stands, and it goes one release staler — recorded, not taken), the archived predecessor, which was not read from. Not lifted, recorded: `estate/README.md:91`, whose "the retrieval-image default by wheel: 0.1.8–0.1.12 default to 0.5.0" stops covering the newest wheel without becoming false — this cut's own residue in a file it did not lift, the CP-97 shape exactly. Recorded beside it and NOT this cut's: `:105`'s "Two measurements, not assumptions", which describes the probe CP-99 tightened — a listener on the gateway port and a dial from inside the run's network — and is now **incomplete** rather than wrong: at 0.1.13 that listener answers a per-run nonce and the host leg is dialled too. And a claim this checkpoint checked and did NOT take: `:35`'s "`host.docker.internal` never resolves in an episode" reads like the flat impossibility CP-99 corrected at `server-guide.md:274` and in `troubleshooting.md`'s trap 2, but this file's first line scopes it to "the host the library was measured on (a shared Linux GPU box, 'the H200')" and the line sits under "The networking facts — `rollout.h200.yaml` holds values, never code"; on that host, with no Desktop `--add-host`, it holds. Row 99 carries this file's residue and these join it. Beside them, also not lifted and also recorded: `estate/mcp-service/README.md:139`/`:143`, whose "134 tests across nine modules" and `# 134 passed` have been wrong by thirty tests and one module since CP-84/85 — the suite is 164 across ten, which this release re-ran and CI proves on every push. **What a stranger gets at 0.1.13 that they did not get at 0.1.12**: an `up` whose gateway-host measurement is one — a sentinel answering a per-run nonce, dialled from a container on the run's network AND from this host, accepting only the candidate that returns the nonce on both legs and recording what answered each; `host.docker.internal` appended rather than tried first on nothing but resolving, which is the single line that would have saved b1's 22 minutes; a gateway port this process cannot bind reported UNMEASURED with the port and `--gateway-host` named, instead of a `measured:` label over somebody else's listener; a `docker exec` that could not start the interpreter reported as `exit 127 via …` instead of as no failure at all; five user-visible labels that no longer write a measurement's word over an inference — three of them rooted in a `reuse_measurement` flag three consumers never read — beside a sixth in a source comment and the probe's own "and from this host" leg, which is dialled now instead of assumed; a `vfs` warning carrying numbers that were measured against a control instead of "~13 GB per container" that never was; a pins skeleton that says `NOT NEEDED on this estate` where the pins in force already cover the corpus, with its own path written out and its printed URL no longer running into the prose; and a fallback probe whose reaper actually waits out its bound. **And [CP-99]'s own closing sentence is superseded here.** "What round six inherits: a 0.1.12 whose probe is corrected in source and not on the wire" was true when it was written and is superseded at publication, not at this commit: from 0.1.13 none of the four behaviours that sentence listed as still-shipped ships any longer, and the demo's `gsj-polar` image is re-cut rather than a generation behind its own `bootstrap.py`. Round six inherits that, not a 0.1.12 — and what it still inherits is the four paragraphs above: the three rows a release cannot close, row 109's launch note, and the residues recorded rather than taken. **Pushed**: the library (release commit, tag), the demo after publication; the examples and the archived predecessor untouched.

**[CP-101] What round six found — four strangers, five accepted episodes, three quarantines, the `bring-your-own` walk performed end to end by someone who did not write it for the first time, twice, by two different routes, and the launch note answered 4/4.**

Round six (2026-09-09) ran four containerised strangers against **0.1.13** and the matched `gsj-polar:f0e8343a-gsj0.1.13`, holding door, agent and model constant against round five, dropping the storage-driver axis (all four `overlay2`, the `vfs` result banked at CP-99) and changing **one variable: the operator's launch note** — row 109's event. Both demo-door strangers finished (a1 two accepted episodes, a2 one); both library-door strangers walked `bring-your-own.md` end to end **unprompted** — a first episode quarantined under the reference pins, the body inspected by hand, `derive_my_pins.py` run on the skeleton, both legs restarted, a second submit accepted, re-verified in a separate process — and they did it by **opposite routes**: b1 ran Polar's two processes from the public image, b2 never pulled it and took the checkout route, because the container recipe lives in the demo repository its door prompt forbids. **The round's best finding belonged to no existing row and is now row 110**: b1's first submit came back `ADM1:status_not_completed:ERROR` on a body that was *whole* — one chain, 4/4/4 merged, `findings: []`, the deliverable on disk — and b1 diagnosed it to the mechanism from the vendored source. `pi_harness.py:289` ends pi's command with `| tee …/pi.txt`; **a pipeline's exit status is its last command's**; Polar `mkdtemp`s its session directory under the *gateway process's* `$TMPDIR` and bind-mounts it into every sandbox; b1's gateway was a container, so that path did not exist on the host and "the Docker daemon, asked for that bind source, silently created an empty host directory and mounted that" — `logs/agent` was absent inside the sandbox, `tee` failed, the step exited 1, and a perfect episode was stamped ERROR. `ls -d /tmp/session-*` on an empty directory was the smoking gun; `-e TMPDIR` was the cure, and the re-run produced exactly the four findings the guide predicts. b1's own sentence is the finding's name: **"The agent did the work; the trace is intact; the status is a lie about it."** That is the fourth occurrence in six checkpoints of one class — CP-96's bound that never ran, CP-98's fake nobody measured, CP-99's label over an inference, and now a status field reporting a subprocess's plumbing as the episode's outcome. The cure taken is documentation, on all three sites b1 filed: `how-it-works.md` step 7's "evidence collection never fails the run" (true of `postprocess`, false of the step one stage earlier), `troubleshooting.md`'s cure for this shape (which pointed at a directory Polar `rmtree`s at every session end — an instruction that cannot be followed, and b1 went looking), and the flag itself, which was **on no library page** because the demo's compose sets it and door A can therefore never meet this. The `gsj_rollout/` fix is **priced and not taken**: capturing pi's status through the pipe is +2 to +4 net lines in a module at 2,034/2,034 with headroom 0, so it needs an ADR raising the law, the machine-checked equality moved with it, and a release — and the three cheaper-looking cures (a plain redirect, `|| true`, `pipefail`) are wrong rather than merely expensive, each for a stated reason.

**Row 108 is answered, and the answer is that naming the image was necessary and is not sufficient** — filed independently by both library-door strangers from opposite routes and **ranked first by both**. b1 reached a working estate only by deriving four `docker run` flags itself ("Commands (mine; no page contains them)") plus a fifth after the ADM1, and quoted this project's own row back at it: CP-99's cure "turns door B's terminal step into a `docker pull` with no clone. **It does not.** It turns it into a `docker pull` plus four undocumented flags." `bring-your-own.md` now carries a `## Polar's two processes` section with **both** invocations — b1's, verified against the transcript rather than invented — one clause per flag, the `polar status` check that the gateway actually *registered*, and b2's checkout route beside it; b1's near-miss rides the `--env-file` clause because it is the one that fails silently (compose strips `KEY='value'`, `docker run --env-file` does not, and the reward for missing it is a green episode with no retrieved pages). `cli.py`'s serve NOTE still names only a `<checkout>` and spells no image while `estate.py`'s names it — b1 hit that disagreement from the door without going looking — and the page now **says so** rather than implying otherwise: the line is inside the size law at zero headroom and stays row 108's residue. **Row 102 is re-pointed rather than closed**, on a better answer than CP-99 expected: nobody opened a skeleton by hand for the second round running, and **both** library-door strangers consumed one through `derive_my_pins.py` — the walk's step 4 *performed*, which is exactly the moment rounds two through five never reached. The skeleton is an input to a script, not a page to be read; what stays open is narrower and now tokened — whether a reader who did not write one can tell a good skeleton from a bad one, since every use so far is a script asserting and proceeding. The contradiction the round did find was in the tool: a1 read `up`'s conditional pins line and, **twenty lines later on the same screen**, the `== run demo ==` footer still claiming `G1/G2 EMPTY until an inspected quarantined episode supplies them`, and filed the pair (its F6, ranked second). That footer was the unconditional site CP-99's amendment did not reach; `skeleton_footer_row` is conditional on the same fact the pins line tests, proven both ways (`test_estate_cp101_round6.py`, 11 tests; corpus suite 483 → 494). **Rows 101 and 109 close.** Row 101 on two independent reproductions from the door that runs the code: `--add-host` was kept deliberately, both `up`s chose the run network's gateway under the both-legs label, b1 read it for what it now says — *"not `host.docker.internal`, which on this box would have resolved … The label means what it says. No finding; a fix working"* — and both `run.json`s record the name as `container: our sentinel, host: nothing, reachable: false`, the address 0.1.12 would have taken, measured and refused. Row 109 on 4/4 daemon introspection with **zero refusals** against round five's 2/2 Codex refusal, with agent, door and model held constant so the note is the one variable that moved — and it closes **carrying its price**: round-six scope-reading is **not comparable with rounds two through five, by construction**, which is the comparability CP-99 declined to spend blind and the operator spent deliberately, with a change record, buying back a whole run. **Row 106 is decided: CLOSED, argued.** Three quarantines, and both readers read the *finding* rather than the summary — b1 ran the page's checklist by hand over the body, b2 **recomputed the wire system prompt's sha256 and matched it to the hash inside the `G2` string**, which is the precise refutation of the sentence that opened the row ("a table in a JSON file describing itself"), because the number came from the body, outside the table. The caveat was weighed and rejected as a reason to stay open: every `G`-finding was predicted by the guide before the submit, but the row's token asked for a refusal *read*, not a refusal *unforeseen*; the strict reading is unfalsifiable in the good case, since it makes accurate documentation the only thing keeping the row open; and the unforeseen refusal happened anyway — b1's `ADM1`, which nothing predicted, which was read, and which produced row 110. Admission is not a `G`-gate, so the literal residue ("no `G`-gate has yet refused something its operator did not expect") is recorded and deliberately not re-opened, with the argument left where a later checkpoint can disagree with it. **Row 107 stays open and its demo half is taken**: three rounds have now failed to produce the t=2/t=4 pair by accident, which is an instrument fault rather than luck, so the demo's walkthrough scripts it as step 1b over the two rows its own corpus has always held — `case_orchard` at t=2 beside t=4, the 1998 easement deed only on page 4 — and a reader who follows the README now produces the evidence as a by-product. The row waits on someone actually running it.

**The demo's list, and what this checkpoint judged was not the software's.** Nine rows taken in the demo (F-106–F-114) and three recorded with their events named (F-115–F-117), all from round six's two demo-door strangers: a **quickstart** above the fence, ranked first of nine by a2, because the prerequisites were a comment *inside* a bash fence 338 lines down and the one they gate is the `docker run --rm alpine true` smoke two earlier runs died on; the **pip timeout flags moved into the copyable line**, asked for by three strangers in the same words, after a1's plain install failed at 190 s and its *documented cure* failed again at **935 s** before an unchanged third attempt landed; **`df --block-size=1M`** replacing a `df -h` that read a flat `102G` twice while a `df -k` sampler on the same filesystem moved **+214 MB/min**, plus the half neither document had (on a shared data root `df` counts every tenant); the **retry signature written as the conjunction it is**, retiring "a healthy one never does" — the claim CP-99 falsified 3/3 and fixed library-side while the demo kept it a round longer; the **storage-driver sentence made conditional**, because `up` warns only on a copy-on-create driver and a2 went looking for a line four `overlay2` strangers were never going to see; **"~6 GB" split into the two numbers it was conflating** (disk after extraction against ~4.05 GB on the wire, the retrieval image 2.65 GB of it) after a reader extrapolated "on the order of a day" from the wrong one; `python3 -m pytest`; and the `--row 0`/`--row 2` disagreement resolved **README-side only**, because the printout's `--row 0` is pinned by demo tests this lift does not open. **What is not the software's, judged and recorded**: the `pyarrow` timeouts are the **rig's** (four concurrent installs on one NAT — a1's link measured 1.87 MB/s while b2 pulled at 2.1 MB/s in the same second; the *documentation* ask is real, the duration is not the README's, and round seven staggers the launches); `PROMPT-B.md` line 12's literal `pip install` failing under PEP 668 is a **standing harness debt**, byte-identical since round two, and it stays that way because the door prompts must remain byte-identical or the door comparison dies with the note comparison — the operator decides; Claude Code's background-task guard reaping a2's `up` twice on a `free` reading while a 4 GB pull filled the page cache is the **vehicle's** (a2 diagnosed it correctly and refused to charge it to the README), and a2's own `until ! kill -0 <pid>` loop over a zombie is the **stranger's** — round four's self-matching-`pgrep` class, a liveness test that cannot come true. **The harness's two misreads of its own strangers name a class, and §8 rule 10 gains clause (e) for it**: a completion matcher that looked for `#`/`**` headings counted 0/4 closing sections in a transcript that had all four in bare capitals, and a validator run against the receiver's quarantine *envelope* rather than the body produced a confident verdict about a document nobody wrote. Both are **green checks on the wrong object** — a real measurement, correctly performed, of something other than the thing claimed — which is CP-98's fake and CP-99's label a third and fourth time. Rule 10's letter did not cover it (it governs a *fake's* shape) and its reasoning did: clause (e) generalises "measured, never written" from a fake's streams to **every shape a check assumes** — a heading grammar, a document's nesting level — and requires a check to record which object it validated. Scope is stated in the clause: both occurrences were in the **untracked rig**, not in `estate/corpus/tests/`, so the suite's machinery did not fail; the clause is added anyway because a test that validates the wrong layer of a nested artifact passes for exactly as long as nobody looks. **Row 105 stays open**: four reproductions, both door-B strangers declared the exposure before measuring, and b1 wrote its own corpus so its G1/G2 would be measurements — those two hashes and b2's G2 are the only new numbers the round produced, and the cure is an endpoint this repository has never published, which is the operator's. **Not touched**: `gsj_rollout/` (2,034/2,034, unchanged — `pi_harness.py` and `cli.py` both frozen and both priced), `vendor/`, `pins/`, `tests/`, `pyproject.toml`, `.github/`, `estate/corpus/ingest_corpus.py`, `estate/mcp-service/`, the examples repo (its trailer keeps F-83 and goes one round staler, recorded) and the archived predecessor, which was not read from. **No release** — a later checkpoint publishes what needs a wheel, and until then a consumer's `up` prints the unconditional footer. **Pushed**: the library and the demo, one commit each.

**[CP-102] Release 0.1.14 — round six's fixes reach consumers.**

0.1.14 is a patch, and the argument is the one 0.1.12 and 0.1.13 needed, with one stdout row added to it. `gsj_rollout/`'s eight modules are byte-identical to 0.1.13's but for `__init__.py`'s literal — measured against the published 0.1.13 wheel, not asserted — the census stays 2,034/2,034 with headroom 0 by design, `checks.py` is byte-identical across 0.1.11 through 0.1.14, both pins sets and the G2 capture are unchanged, and the artifact is the same eighteen entries with the same seven required paths and no banned prefix. What moved is the packaged `estate.py` (sha256 `c77d4f2f…` against 0.1.13's `e3d3d8d6…`), and what moved in it is CP-101: `skeleton_footer_row`, the `== run <name> ==` footer's skeleton line made conditional on the same fact the pins line tests. **That is the one shape a consumer could be parsing, and it is priced.** On an estate whose pins in force cover its corpus, 0.1.13 printed `G1/G2 EMPTY until an inspected quarantined episode supplies them (#your-pins reads it)` twenty lines under a pins line saying `NOT NEEDED here` — the contradiction a round-six stranger filed — and 0.1.14 prints `G1/G2 EMPTY in it — NOT NEEDED on this estate: <pins path> already carries them (#your-pins is for a corpus or model those pins do not cover)`; a script matching the old sentence breaks on exactly the branch where 0.1.13 was contradicting itself. On a corpus the pins do not cover the bytes are unchanged, the `== run <name> ==` markers do not move (CP-101's `test_estate_cp101_round6.py` pins them and this release does not touch it), `run.json`, the skeleton's `format` and every other line of `up`'s stdout are unchanged, and no documented consumer matches that row — the demo's `bootstrap.py` parses `up`'s stdout only for the `== run <name> ==` header and the `next — ` line (`bootstrap.py:1161–1162`; both kept byte for byte, which is why they are the contract) and takes its facts from `run.json`. No minor bump. Measured echoes, each against the tree and never copied from a register row: the shelf holds 42 ADRs (`ls docs/decisions`; ADR-0042 twice amended, none added at CP-101, so `CLAUDE.md`'s 0001–0042 and `docs/README.md:37`'s 0027–0042 stand); twenty-five changed lines across twelve files, twenty-two of them carrying the literal (the two literals and the test pin, `CLAUDE.md:5` and `:52`, the `gsj-polar` image tag on `README.md:20`, five lines of `bring-your-own.md` and one of `troubleshooting.md` — the same pre-publication window CP-100 stated, closed inside this checkpoint by the cut — the wheel sentences on `README.md:63`, `docs/guide/README.md:23` and `trainer-guide.md:9`, the deprecated pipeline entry retained through 0.1.14 on four pages, the alt text's "unchanged layout through 0.1.14", and `trainer-guide.md:22`'s per-wheel history gaining its 0.1.14 clause); and the CI corpus label re-set from 483 to what the runner collects — 494, which is 493 passed plus the demo-thirty skip, stale by eleven since CP-101 because `.github/` was frozen there, exactly as CP-98's 463 → 483 was. `CLAUDE.md:74`'s sentence that said the label still read 483 is rewritten to say when it moved. The acceptance gates, as at CP-95, CP-97 and CP-100, plus what CP-101 adds: the eighteen-entry artifact asserted locally on the built wheel before the push, with the packaged `estate.py` and `ingest_corpus.py` sha256-equal to their frozen sources; the three suites (176 / 494 / 164); CI green in all four jobs with the label matching what it collects; `workflow_dispatch` rehearsed to TestPyPI before the tag; both public indexes installed by name from a scratch venv outside every checkout; the standing seventeen consumer checks and the CP-88/90, CP-92 and CP-94 seams from both installs; **and each CP-101 fix driven from the installed artifact rather than the checkout** — the footer's both branches on two real estates in one box (the covered estate printing `NOT NEEDED on this estate`, a scaffolded corpus printing 0.1.13's sentence byte for byte) with the pins line and the footer of the same `up` compared, a1's F6 closed at the site that ships; b1's two `docker run` invocations run as written from the published page, the secret's length read back as 64 and `polar status` reporting one registered node; row 110's `TMPDIR` failure reproduced from the installed wheel (`ADM1:status_not_completed:ERROR` on a whole trajectory) and cured by the page's flag (`collected 1/1`); the `mkdir -p runs` refusal met and cured by the page's line; and one accepted episode from the installed artifact read back. The ids, the wheel's sha256 on both indexes, the run ids and each job's colour are in this paragraph's record clause and the printed CP-102 report.

**What 0.1.14 does not carry, row 110 first and said as plainly as what it does.** **Row 110 is routed around, not fixed.** `pi_harness.py:289`'s `| tee` still makes the pipeline's exit the step's, so a containerised gateway without a same-path `TMPDIR` still throws a whole trajectory away as `ERROR` — the page routes around it with a flag, and nothing in the wheel fixes it; the fix stays priced at +2 to +4 lines behind an ADR raising the size law and the moved equality test, and this release is not that sitting. `gsj_rollout/cli.py`'s serve NOTE still names only a `<checkout>` where `estate.py`'s names the image — row 108's residue, frozen at zero headroom, and the page says so rather than implying otherwise. `ingest_corpus.py`'s standalone `verify` headline still counts a skip as a pass (row 97; the file rides the wheel byte-unchanged, and publishing it discharges nothing). `status`'s partial path still prints a recorded engine value in probe tense (CP-99's finding, unfixed by the two cuts since). And four rows a release cannot touch stay where CP-101 left them: **105** needs an endpoint this repository has never published; **106** and **109** closed at CP-101 and a cut does not re-open them; **107** waits on a stranger running the demo's step 1b, which this checkpoint runs itself and reports without claiming it as the row's event; **102**'s narrowed question — a reader who judges a skeleton rather than consuming one — is a round's.

After PyPI publication, and only then, the demo's single commit moves its floor to `>=0.1.14`, its remedy strings, its `polar.Dockerfile` default and its three `docker run` recipes (the newest, step 1b's, is CP-101's); A-28's rebuild duty fires because the packaged estate payload changed: `gsj-polar:f0e8343a-gsj0.1.14` re-cut two-platform from the public `v0.1.14` and PyPI 0.1.14, both children pulled anonymously and reporting 0.1.14, the pulled image's packaged `estate.py` sha256-equal to the wheel's, a fresh clone run to an accepted episode read back on that image, **and the walkthrough's step 1b pair run on it** — row 2 beside row 3, `case_orchard` at t=2 and t=4, what the two transcripts show recorded in the printed report. The mixed state CP-100 ended does not reopen: the demo's `bootstrap.py` and the image it pins move in the same commit, after the wheel exists. F-87 and F-92 stay parked on the export format bump: `read.py` is untouched by this commit, no export is re-shaped, and a floor bump and an image cut are still not the demo sitting that opens it — CP-96's, CP-97's and CP-100's reading holds and is checked against `git show --stat`, not inherited. Not touched: the examples repo (its trailer's F-83 stands, and it goes one release staler — recorded, not taken), the archived predecessor (not read from), `estate/README.md:91`'s retrieval-image-default range (stops covering the newest wheel without becoming false, the CP-97 and CP-100 shape, row 99's), and `estate/mcp-service/README.md:139`/`:143`'s stale suite count (wrong since CP-84/85, recorded at CP-100, still outside the lift). Next fresh: **row 111** in the library's register, **F-118** in the demo's, and F-83 stays the examples trailer's retained marker. **What the final round meets that round six did not**: an installed `up` whose footer agrees with its pins line; a library-door page that runs Polar's two processes from the named image, flag by flag, with the `TMPDIR` and `--env-file` traps written where they bite and a `mkdir -p runs` above the `up` line; a `troubleshooting.md` whose cure for `step <i> exited with code <rc>` can be followed; a demo whose walkthrough scripts the t=2/t=4 pair as step 1b on an image that carries this wheel; and the same two decisions that are the operator's alone — staggered launches, and `PROMPT-B.md` line 12.

**[CP-102, record] What happened, per repository, stamped rather than remembered.** Library: the release commit `8fb7207` pushed, CI 34369393141 → green in all four jobs (`root suite (176)`, **`corpus suite (494)`** — the runner's log reads `493 passed, 1 skipped in 48.68s`, so the label matches what it collects — `mcp-service suite (164)`, `wheel + packaged-pins install proof`); the rehearsal 34369408538 (`workflow_dispatch`) → build ✓ TestPyPI ✓ PyPI skipped by the tag gate; tag **`v0.1.14`** on `8fb7207`, its run 34370202067 → tag-matches-version ✓ build ✓ TestPyPI ✓ **PyPI ✓**. One wheel, one sha256 — `78976197723a2e14…` — built locally, on TestPyPI (2026-09-09T15:19:29Z) and on PyPI (2026-09-09T15:26:57Z); both indexes installed by name from scratch venvs outside every checkout (each index's first `pip install` hit the index lag and the unchanged second attempt landed — `install.attempt-1.log` kept for both). **Each CP-101 fix, from the installed artifact**: in a nested Linux box on b1's topology (Ubuntu 24.04 aarch64, its own `dockerd` on `overlay2`, the non-root user `stranger`, PyPI 0.1.14 by name, the macOS engine through CP-96's relay) — `up` without `./runs` refused (`estate: REFUSED — cannot prepare the run lock directory … expected: an existing writable runs directory at /home/stranger/runs … create that root directory first for a new estate`, exit 1), the page's `mkdir -p runs` cured it; a scaffolded corpus printed `G1/G2 EMPTY until an inspected quarantined episode supplies them (#your-pins reads it)` — 0.1.13's bytes, compared to the CP-101 evidence log — and the staging corpus printed `G1/G2 EMPTY in it — NOT NEEDED on this estate: /root/venv/lib/python3.12/site-packages/gsj_rollout/pins/pins.gsj.json already carries them (#your-pins is for a corpus or model those pins do not cover)` under a pins line reading `NOT NEEDED here as far as this script can check` twenty lines up, a1's F6 closed at the site that ships; the published page's Route A fence, fetched from `main` and run as written: without `-e TMPDIR` the first submit came back `rejected sk-polar-53006ffa-3ecf-4ee8-b117-6a0aa4982433: ['ADM1:status_not_completed:ERROR'], collected 0/1, exit 1` on a body whose status was `ERROR` with `status `ERROR`, error `step 0 exited with code 1`, one whole chain in the trajectory — b1's shape, and `/tmp/session-sk-polar-idevx441 -> 0 entries` on the box's host as the smoking gun, the gateway log reading `transcript download failed: docker cp download_file failed with exit code 1`` — row 110 from the installed wheel — and the fence verbatim gave `collected 1/1 episodes, exit 0`, the secret's length **64** inside the gateway container (the unedited `.env` through `--env-file` reads 66), `polar status` → `Registered Nodes: 1`, and the episode `sk-polar-a220a5b8-7a…` read back in a separate process: `COMPLETED`, `validate_session_result` → `[]`, receiver and trainer bytes equal. The standing checks: `PROOF OK — seventeen consumer checks green: [0..17]` from both indexes, with the artifact assertions inside it (all eight `gsj_rollout/*.py` byte-identical to the checkout's, census 2,034 read from site-packages, packaged `estate.py` `c77d4f2f…` and `ingest_corpus.py` `a291ec56…` equal to their frozen sources), the CP-88/90 seams, CP-92's 28 with the five wheel warnings resolving to three real anchors on the published page, CP-94's 21, and CP-100's live script from the PyPI install — the skeleton's conditional prose, the reaper's 6 s bound engaging, a port in use UNMEASURED, exec-127 a failure, the fifth provenance value, and a second accepted episode `sk-polar-0fb89585…` (426 trainable of 4,633, 7 tool calls) read back. Demo: after PyPI (2026-09-09T15:26:57Z) the edits began at 2026-09-09T15:27:53Z; `gsj-polar:f0e8343a-gsj0.1.14` built two-platform from public `v0.1.14` and PyPI 0.1.14 (859 s) — index `9526f3f7…`, amd64 `a7a498ab…`, arm64 `dab8d7bf…`, both children read and pulled anonymously, each reporting 0.1.14 and `8fb7207`, the packaged `estate.py` `c77d4f2f…` byte-equal to the published wheel's, `skeleton_footer_row` called inside the container on both branches; the demo commit `4f7b32f` pushed, its suite 59 passed on the plain PyPI 0.1.14 venv; the fresh clone's walkthrough on that image — `up` printing `NOT NEEDED on this estate` in its `== run demo ==` footer, row 2 → `sk-polar-5f6705d8-ed…` accepted, **step 1b** row 3 (`--task-id easement`) → `sk-polar-e2a5f0f7-a7…` accepted, both validated to `[]` in a separate process. **What the pair showed**: the checkout half and not the retrieval half. Row 2 (`case_orchard@2`) checked out `timestep-2` with pages 1..2 visible, called `mcp_gsj_search_case` and got pages [2, 1] — `all <= timestep 2 (the cutoff holds)`, which at t=2 is the README's own point that two pages cannot show WHERE the bound sits; row 3 (`case_orchard@4`, `--task-id easement`) checked out `timestep-4` with pages 1..4 visible — the 1998 easement deed on page 4 now in the workspace — and the 0.6B floor model **never called `mcp_gsj_search_case`**: its prompt ("Does any recorded easement or right of way affect the disputed strip? Name the registry number and cite the page") took it to `mcp_gsj_search_decisions` alone, five precedent hits, and an answer citing `GREV000082013` and `page:7` — a page no checkout holds and no hit returned (`search_pages_returned: null`, `pages_beyond_timestep: []`, both episodes accepted, both `findings: []`). So the pair as scripted demonstrates the checkout scoping (2 pages against 4) and leaves the retrieval bound unexercised, because the instrument depends on the agent choosing the case-file tool at t=4 and the README's "the t=4 episode can find, read and cite it" is a *can*; recorded on row 107 and left for the next demo sitting to decide (no mint — F-118 stays next fresh). Examples: untouched, clean, F-83. Predecessor: untouched, not read from, clean. The final round inherits what the paragraph above says, and nothing this record contradicts.

**[CP-103] The instrument before the last round — one demo sitting, no release, no round.**

CP-102 ran the walkthrough's step 1b as its author and found the pair demonstrating the checkout half only: at t=4 the 0.6B floor model never called `mcp_gsj_search_case`, because row 3's prompt ("Does any recorded easement or right of way affect the disputed strip? Name the registry number and cite the page") was a question a precedent search could plausibly answer. Round seven is the last round, and an instrument whose evidence depends on which tool a floor model reaches for closes row 107 on luck or not at all — so this sitting fixed the instrument and then ran it. **Measured first, changed second.** One exploratory estate from the demo's tree, the shipped prompt and three re-wordings through the direct `--case/--timestep/--prompt` submit (no regenerated bank needed), five samples each at t=4 and the survivors at t=2 — 37 episodes, every one accepted: the shipped prompt opened the case file in 1 of 5 and went to the decisions tool alone in 4 (a docket as the "registry number", `page:7` in three answers) — CP-102 saw the mode, not bad luck; two re-wordings carrying an "if the file does not contain one, say so" clause let the floor model write that sentence with **no tool call** in 3 of 5 and 5 of 5 samples, at the timestep where the deed exists — the honest-empty clause is a lazy exit for a 0.6B model, which is why the row-3 prompt does not carry one; the third — *"Is there an easement deed in the case file so far? If so, which page records it, what is the deed number and when was it registered? Cite the page as (page:N)."* — called `mcp_gsj_search_case` in 5 of 5 and every answer named deed 98-4417, 11 August 1998 and page 4. The prompt names the case file as the source (row 2's own "so far" anchor), asks for facts that live on page 4 alone (the demo's suite now checks the thirty rendered decisions do not carry the deed number — `decisions.json`'s `bears_on` notes do, and are never rendered), keeps the `(page:N)` form the agent is taught, and names no tool. At t=2 the same wording searched in 5 of 5, got pages 1 and 2 back and could not name the number — and every sample then wrote that the deed is "recorded on page 1", two inventing a 2019 registration: the search result and the absent number are the retrieval bound holding, the sentence is the floor model confabulating over it, and the demo's README says exactly that rather than promising an honest negative the model does not produce. **Changed** (demo commit `04bd418`, four files, +373/−31): `synthetic/make_corpus.py` (the prompt, its argument in a comment, the generator's own "must find it" made "can find it — and row 3 asks for it as a case-file question"), the README's step 1b (the t=4 transcript's expected shape; what a reader checks when the agent goes elsewhere — `search_pages_returned: null`, the case file never searched, the episode accepted regardless; the same question at t=2 through the direct triple, fenced), `tests/test_corpus_cp103.py` (13 tests: the bank's row order, the prompt's shape, the deed number's single home, the F-117 lock probe measured on the real function; the demo suite 72 passed), and, because the lift opened the README and their events name exactly that, F-115–F-117 taken from measurements: the roster is seven pi built-ins and four MCP tools (counted from `trajectory.traces[0].tools` of an archived episode — the README said eight and three and then named a fourth); `response` is the session's whole merged chain and the 8,192 cap is per completion (`pi_harness.py:130`'s `maxTokens`; the CP-102 export's two turns span `[0, 34)` and `[2480, 2522)` of a 2,522-token response); a lock file whose process is gone holds no `flock`, so `status` ignores it and it never needs removing by hand. Demo `F-118` (the prompt, FIXED), `F-119` (step 1b's elsewhere paragraph, FIXED) and `F-120` (a `page:N` grounding check beside the `dec:` one — priced at 40–60 lines of `read.py`, a fixture, tests and an export format bump, parked with F-87/F-92; not built, `read.py` not lifted; and noted that it would have graded the t=2 "page 1" confabulation *grounded*, because page 1 was returned — it checks retrieval, not truth) minted; next fresh F-121. **Proven, from a fresh clone of `04bd418` on the published `gsj-polar:f0e8343a-gsj0.1.14` (index `9526f3f7…`), the README's own install line into a venv of its own, `up` 72.7 s**: row 2 → `mcp_gsj_search_case`, `2 hits, pages [1, 2] — all <= timestep 2`; **row 3 from step 1b's fence verbatim** (`--task-id easement`, 14.9 s) → `mcp_gsj_search_case {"query": "easement deed"}` → `4 hits, pages [4, 1, 2, 3] — all <= timestep 4 (the cutoff holds)` → *"The easement deed in the case file is recorded on page 4, deed number 98-4417, and was registered on 11 August 1998."*; the same question at t=2 through the triple (23.9 s) → the same search returning pages 1 and 2, no 98-4417, "the deed number is not provided", and the page-1 confabulation the README names. All three `COMPLETED`, `finish stop`, `validate_session_result → []` in a separate process; the estate down, the container inventory byte-for-byte restored. The first proof attempt completed the pair and stopped before the t=2 episode on the author's own script — an argument-order self-check one index off (CP-100's lesson 8, met again); fixed, the first attempt kept, the whole proof re-run from a new clone. **Row 107** stays OPEN with its token unchanged and its event not re-worded — the pair now exercises both halves on the floor model, so a stranger following step 1b produces evidence a reader can read; what the row still waits on is that stranger. Nothing in `gsj_rollout/`, `estate/`, `vendor/`, `pins/`, `tests/`, `pyproject.toml` or `.github/` moved (`git diff 301f7cf --` over them is empty; census 2,034/2,034; root 176, corpus 494, mcp-service 164 passed); the demo's `bootstrap.py`, `preflight.py` and `read.py` are byte-identical to `4f7b32f`; the examples repo and the predecessor untouched; no H200, no round-seven setup, neither door prompt nor the launch note read or edited.

**[CP-104] The last round — round seven, 2026-09-09, four containerised strangers against 0.1.14 and the matched `gsj-polar:f0e8343a-gsj0.1.14` — and what the register looks like now that no further round is coming.** Two doors, two agents each, all four on `overlay2`, launched headless and staggered by eight minutes: **ten accepted episodes** (a1 4, a2 3, b1 2, b2 1) and **two quarantines** (b1 one, b2 one — both `COMPLETED` bodies whose findings `bring-your-own.md` predicted, the reference recomputation equal to the envelope's, both read by their strangers before deriving). **Both doors complete**: every stranger reached an accepted episode, door B from PyPI plus the pages and the public Polar image — b1 by Route A's two `docker run` invocations with `-e TMPDIR`, so row 110's `ADM1` did not recur, and b2 by the same route rather than round six's checkout. This checkpoint's prompt said three quarantines; the collected bodies hold two, and this record says two. The four transcripts were each read whole by one reader and a completeness critic, and all 167 findings extracted were checked against the transcript by an adversarial verifier: 16 refuted, every one a framing or a critic's own addition and none a stranger's filed finding. Two of the refutations corrected this checkpoint's first drafts before they landed: b1's "thirty retries" are three backoffs of 5, 10 and 15 s in its own log, and a reader who passes `--end-of-turn-token-id` still gets `up`'s own measurement printed beside it. **Rows decided.** **107 CLOSED** on the bodies — a1 ran step 1b's fence at t=4 (page 4 the top hit of a case-file search, `(page:4)`, 98-4417, 11 August 1998) and the same question at t=2 through the triple (five searches, pages 1 and 2 only, *"No, there is no easement deed recorded in the case file so far."*), a2 ran the t=4 half the same way, and b1 ran a t=2/t=4 pair on a corpus of its own. The closure is about what the walkthrough shows, never about whether the service filters, and the demo's step 1b now says where each half is enforced (demo F-124), checked against the live service: on the demo's own estate from a fresh clone of `4a15082`, one query under a t=2 and a t=4 token returned pages `[1, 2]` and `[4, 1, 2, 3]`, and a t=2 token re-encoded to `T=4` was refused 401. **102 CLOSED, its residue recorded** — b1 tested the skeleton against its documented refusal, got the admission findings from `{}` first and the refusal from a real body second: judgement, weighed against its having read this row first. Nobody has declined a skeleton or caught one disagreeing with their estate, and with the rounds over that is recorded rather than awaited. The ordering b1 tripped on — admission before pins — is now stated beside every skeleton refusal, the skeleton's own prose included. **105 RECORDED AND UNMEASURABLE HERE** — the endpoint is unchanged, both door-B derivations were declared reproductions, and both door-B strangers had read the register before beginning (b1 all of `docs/`, b2 three files because the README says "read first"). That is reading the documentation the door permits, weighed on each row it touches; the rows keep describing the experiment they recorded, which is over. **108** annotated: its `estate.py` half is answered in source, its `cli.py` half stands. **106 and 109**: their superseded round tokens struck. **111 NEW, DONE IN SOURCE** — b1's findings 1–4, 6, 7 (the estate tool's half) and 8, b2's four, and a2's FINDING 3 where it reaches `measure_tail`, in `estate.py` and five pages, proven on a real estate from a locally built wheel of this tree: from `python -m gsj_rollout.estate up` of a scratch staging copy (80.8 s) the closing block listed the bank's rows and printed the pullable image, `status` named a run cut in its first phase instead of `no run named`, a real pull's heartbeat aged its tally and qualified its verdict, and one episode was accepted and validated `[]` in a separate process; consumer delivery is the next release's. **112 NEW** — the residue in files this lift did not open (`ingest_corpus.py`'s validate table, `cli.py`'s help and `--out`, `estate/README.md:101`, the CI corpus label). The demo's register: **F-121–F-128** — the image sizes measured from the manifests, `bootstrap.py`'s failed-derivation path (the README half taken, the `bootstrap.py` half recorded), the enforcement path, seven sentences corrected, four questions answered, two recorded for later sittings — next fresh **F-129**; the examples trailer keeps F-83. **What the rig found in itself** stays the round's record's (`round7/REPORT.md` §9): the stalled npm and its racing retry, the watcher that never started (`setsid`), the launch gate's one refusal, the print-mode ceiling that killed a2's first session mid-pull and its mirror that held the finished session open on its own monitors, the exit monitor's replayed baseline, the three-shape closing matcher, the artifact glob. None is a finding against the software and none is taken here; a2 recorded the first as "not the demo's fault, and not mine" and used the kill to test the stale-lock sentence, which held. **What the register looks like now that no further round is coming.** No live `waiting-on:` token in either register waits on a stranger round: the five that did — 102, 105, 106, 107 and 109 — are closed, retired or struck here, and row 10's "a real stranger hit" is the reopen condition of a row closed at CP-40, which a consumer's report meets as well as a round would. Everything still open waits on an event that can still happen: the next release (row 111 and the release-delivery tokens), a freeze-lift of a frozen file (`gsj_rollout/cli.py` for rows 108 and 112, `pi_harness.py` for 110, `ingest_corpus.py`, `estate/mcp-service/`, `estate/corpus/staging/`, `.github/`), the first production bring-up (rows 9, 24 and 38; §7 row 22), a command on the H200 (93), a Polar re-vendor (16, 86), an operator's or a corpus author's decision (rows 19, 50, 70 and 73–75; demo F-82), or a consumer's report (rows 22 and 63, and 105's residue). Thirteen older closed rows still carry a superseded token beside their own `nothing — closed` (36, 37, 40, 48, 49, 51, 52, 53, 60, 61, 65, 66, 69); none names a round, and whether to strike them is the shipping pass's call. The stranger evidence is closed; what it measured stays in the rows that recorded it. Proven: proof A (the library, from a locally built wheel of this tree — nothing published) and proof B (the demo, from a fresh clone of `4a15082` on PyPI 0.1.14 and the published image) each passed every check and left every durable container on the host intact; root 176, corpus 530 (494, the round-seven module's 35, and one from the CP-92 fixture split), mcp-service 164 and demo 89 (72 + 17) passed. Nothing in `gsj_rollout/`, `vendor/`, `pins/`, `tests/`, `pyproject.toml`, `estate/corpus/ingest_corpus.py`, `estate/mcp-service/`, `estate/serving/` or `.github/` moved (`git diff be7bfd0 --` over them is empty; census 2,034/2,034); the demo's `bootstrap.py`, `preflight.py` and `read.py` are byte-identical to `04bd418`; the examples repo and the predecessor untouched; no release, no H200, no round set up.
...
**[CP-105] The front door, and release 0.1.15 — written before the proofs run; the record follows publication.**

No round and no findings sitting: the page a reader meets before deciding to use this, and the release that carries CP-104's `estate.py` to them. **The front door.** The README now opens with what these traces are for — sound now for provenance work (the cutoff, the pins, the reconstruction: every claim the validators make), with the sampling policy (row 22), the codec and the reward (phase 6, deferred at CP-88 because the page grader pays for form) still the consumer's, and what has been trained as the record states it: single optimizer steps (CP-17, CP-21, CP-26, CP-32, CP-87's full-archive step) and two two-step runs (CP-69, CP-87), none of them a training result — the first draft counted two loops and one run, and the checkpoint's own review caught the undercount before the push. Every fact in that paragraph was already in the repository; it moved. The Install section is three routes labelled by what each gets you — a trainer client that runs no episodes, an estate with no Polar, a server that runs episodes (the published image or a checkout) — with the venv line as the command and the bare `pip install` as the note; one fact on it is new, measured here on `ubuntu:24.04`: stock 24.04 lacks `python3-venv`, so the venv line fails until it is installed. **The number.** 0.1.15 is a patch, on the argument 0.1.12–0.1.14 needed: `gsj_rollout/`'s eight modules byte-identical to 0.1.14's but for `__init__.py`'s literal (2,034/2,034), `checks.py` and both pins sets unchanged, the same eighteen entries and seven required paths; what moves is the packaged `estate.py`, CP-104's, whose new stdout lines (the bank's rows, the image NOTE, the tally age) sit inside the `== run <name> ==` block and the refusals, with that header and the `next — ` line — the demo's only parse points — unchanged. **Step 4's window, decided (ADR-0043):** the closing block could not be taught to say its tag may lag (`estate.py` is closed), so the image comes before the front door — the A-28 row above. **Row 112, in part:** the CI corpus label 494 → 530, what the job collects; the guide's round-seven residue corrected by a swept and adversarially verified pass (first use is the first hash gate a trace reaches, whatever the body's status — six sentences; the bank's rows capped at twelve; `status`'s fourth answer; the three-way pull split; the heartbeat's tally; the image route on three pages); two more recorded against `estate.py` as row 112 (f) and (g). The thirteen closed rows' superseded tokens struck. Echoes measured against the tree: the shelf holds 43 ADRs (ADR-0043 added), the label 530, the literal on its release sites, historical mentions kept. **The gates**, as at CP-102 plus this release's: the artifact asserted locally before the push; the three suites (176 / 530 / 164); CI green in all four jobs on the pull request; the rehearsal to TestPyPI before the tag; both public indexes installed by name from scratch venvs outside every checkout, the standing checks and seams from each; each row-111 fix driven from the installed artifact; the three Install routes from the published README, the trainer route shown not running an episode; the image and its window; one accepted episode read back; the demo's fresh clone on the re-cut image, step 1b included. ~~`waiting-on: the CP-105 record commit — run ids, both indexes, the image, the window, the demo — written after publication`~~ (written below — struck at CP-105's record commit)

**[CP-105] The record, after publication — the arc's closing entry.** **What shipped.** 0.1.15, a patch: `gsj_rollout/`'s eight modules byte-identical to 0.1.14's but for `__init__.py`'s literal (2,034/2,034, headroom 0), the wheel's 18 entries and its seven required present, the packaged `estate.py` CP-104's (row 111: the image NOTE with A-28's tag, the bank's rows in the closing block and in `status`, the heartbeat's tally age, the `transfer` kind, the first-phase `status`, the printed commands with `--runs-dir`, `--corpus` and the interpreter, the `/tokenize` retry, the skeleton's first-use clause). Release commit `d3f7120` tagged `v0.1.15`; runs [34514360090](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34514360090) (CI on the pull request, four jobs, corpus label 530), [34514359300](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34514359300) (rehearsal), [34515408106](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34515408106) (tag), [34517274823](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34517274823) (CI on `main`); one sha256 `28aa1b8c…` across the local build, TestPyPI and PyPI. `ghcr.io/mhganainy/gsj-polar:f0e8343a-gsj0.1.15` cut two-platform at public `v0.1.15` and PyPI 0.1.15 — index `24cbe2ea…`, amd64 `cb8786d4…`, arm64 `78609290…` — both children pulled anonymously and reporting 0.1.15, the release commit, and the published wheel's `estate.py` byte for byte. **Step 4's window, decided (ADR-0043) and measured:** `estate.py` is closed, so the closing block could not be taught to say its tag may lag; instead the release commit stayed off `main` — on a branch, CI on its pull request, the rehearsal dispatched on it — until the image pulled. PyPI published at 18:37:45Z; the image index answered an anonymous reader at 18:53:08Z, 15 min 23 s later; `main` moved at 18:55:18Z. In that window the PyPI page and an installed `up` named a tag that did not yet resolve (the only residue, bound to the wheel itself); `main`, every guide page, the page `up`'s NOTE links to, and the demo never did. Demo `34d9d8c`: floor `>=0.1.15`, the image pin, and its own front door.

**What a reader meets now.** The README opens with what the traces are for: sound for provenance — the cutoff (a forged claim refused 401; G5 from the trace alone), the pins (prompt, cards, roster, tail, settings), the reconstruction (the golden pairs, exact) — with the sampling policy (§7 row 22) and the codec not in the trace, the estate's to record and the consumer's to account for, and the reward (phase 6, deferred because the page grader pays for form) the trainer's, and what has been trained stated as the record states it: single optimizer steps at CP-17, CP-21, CP-26, CP-32 and CP-87, and two two-step runs, CP-69 and CP-87 — no training result. Every fact in that paragraph was already in the repository; it was moved, and the checkpoint's review traced each to its source. The Install section is three routes labelled by what each gets you — a trainer client that runs no episodes, an estate with no Polar, a server that runs episodes (the published image or a checkout) — with the venv line as the command and the bare `pip install` as the note; the one new fact on the page, measured here on `ubuntu:24.04`, is that stock 24.04 lacks the venv package. Each route was run from the published README (the `v0.1.15` tag's, byte-identical to `main`'s once it moved) in a nested Ubuntu 24.04 box on b1's topology: route 1 verbatim installs 0.1.15 and has no Polar — `polar` not on PATH, not importable, its `submit` exit 3; route 2 stands the estate, prints the bank's rows and the image, and its `submit` exits 3 too; route 3a pulls the image anonymously and runs the page's two invocations to an accepted episode (route 3b, the checkout recipe verbatim, to another (`sk-polar-13d2d0f9…`) once the printout's own `<secret>` placeholder was filled from the run's `.env`; and on a stock `ubuntu:24.04`, route 1's first line fails as its comment says it will unless `python3-venv` is installed first, and installs 0.1.15 when it is). The demo's front door says what an accepted episode is and is not, and its "Run it" opens with the floor, its forty-line chronicle moved to a closing section (F-127, taken).

**Row 112 split, and two more recorded.** Taken: the CI corpus label (494 → 530, what the job collects). The guide's round-seven residue corrected by a swept, adversarially verified pass: first use is the first hash gate a *trace* reaches, whatever the body's status — an `ERROR` body carrying a trace is refused like a `COMPLETED` one (measured on the installed 0.1.15; six sentences CP-104 wrote otherwise, and row 102's closing note corrected); the bank's rows capped at twelve; `status`'s fourth answer; the three-way pull split; the heartbeat's tally; the image route on how-it-works, the guide index and the roles table. Stands, each on its own lift: `ingest_corpus.py`'s table (a), `cli.py`'s help and `--out` (b, e — with rows 108 and 110), `estate/README.md:101` (c); recorded as (f) and (g), both `estate.py` and measured: the skeleton's own "COMPLETED body" clause, and a first-phase `status` that names Forgejo for a run refused at the sandbox-image check. The thirteen closed rows' superseded tokens are struck (CP-104's hygiene finding). Library next fresh row 113; demo F-129; the examples trailer keeps F-83.

**What this project has proven.** That Polar can own episode execution and trajectory reconstruction behind a 2,034-line shell, and that the traces it produces are what they claim to be: the timestep cutoff held twice and audited once; the pinned prompt, cards, roster, tail and settings checked by the same code on both sides of the wire; `loss_mask` exact and `prompt_ids` byte-identical against an independent golden reference on two platforms; the logprobs real captures; two trainers each taking a real optimizer step on those traces without a server change; a second model family through the same code; and, across seven stranger rounds, readers with nothing but the published artifacts reaching accepted episodes through both doors.

**What it has not.** Learning: single optimizer steps and two two-step runs, whose reward rises the record reads as format uptake — near-verbatim copying at CP-69, a templating policy at CP-87. The sampling policy on the trace: no OpenAI-compatible API reports it, and per-episode binding of a trace to its engine identity is §7 row 22's open half. A reward worth training on: phase 6 waits on a page grader that checks reading, not headings. Concurrent collection and training, and weight sync at cadence: zero data points (A-13). A reasoning geometry other than Qwen's: one foreign family is one data point. And three known defects priced and untaken — row 110's `| tee` status (routed around with `-e TMPDIR`), row 108's `cli.py` NOTE, row 112's residue.

**What the first person to stand an estate up owns that this repository does not.** The engine — its serve argv and generation config are the sampling policy, and nothing on the trace will say what they were; the pins for their own corpus and model (the walk derives them from a first quarantined episode; the wheel's are this estate's); Polar's two processes, from the published image or a checkout, and `-e TMPDIR` if the gateway runs in a container; a reward, and everything a trainer owns — storage, mixing, staleness, weight sync; and the per-episode provenance row 22 names, which the first production bring-up was always going to have to build. This entry closes the arc; if it has a successor, the operator opens it.

**[CP-106] Release 0.1.16 — obligations recorded before publication.** The operator approved the exact narrowed estate-phase candidate at `bda8e55c06e6f71e99b265fb49986acb7fb99206`, production diff SHA256 `eed5cbdb17226f23f47a8c571392a5ee332bb3f9295d6ee9d123ed5c1d9c8a46`. It shares corpus subprocess construction between `up` and `update` and separates private verification findings/exit calculation from child formatting. Caller diagnostics, state writes and recovery checkpoints stay in place; the active `run_phase`/`verify_headline` presentation adapter, raw return values, warnings, marker behavior and subprocess isolation stay unchanged. Production is +57/−35, not a size reduction: estate 5,631 plus pipeline 2,330 lines, outside the unchanged 2,034-line shell and 528-line checks equality. Eleven new tests bring corpus collection to 541; the current CI label follows that collection count, with the workstation-only fixture still skipped on hosted CI. A patch version is appropriate because no public interface, configuration, persisted format, resource, dependency or runtime behavior changes. The accepted implementation and test bytes are frozen; this release adds only version/reference/status records. Candidate B and its census ADR remain independent, unscheduled and unaccepted; known defects and row 24's production-owned obligation remain untaken.

The release gates remain the established four-job CI on the exact release tree, local wheel/sdist inspection and packaged resources, real TestPyPI rehearsal followed by tag publication to TestPyPI/PyPI (wheel only), fresh neutral by-name installs on both public indexes with all seventeen checks and installed seams, the applicable installed live/printed-command and nested routes, and the whole no-TMPDIR ERROR/ADM1 trajectory followed by the documented cure and independent validation to `[]`. Additional live/nested characterization retains its recorded PyPI-only scope; existing installed missing-runs refusal and mkdir/success coverage remain sufficient without another same-run nested-pair requirement. A-28 and ADR-0043 require the matching public-source/PyPI-wheel image and anonymous two-platform proofs before main advances, then the existing demo's exact tested release commit, clean-clone t=4/t=2 proof and push. No completion was claimed by the release-source entry. ~~`waiting-on: CP-106 release completion record after publication, image, main, demo and required proofs succeed`~~ (written below after those obligations succeeded; release source/tag remains separate from this later record).

**[CP-106] Completion record — 2026-09-11, after publication, image, main and demo.** Accepted feature `bda8e55c06e6f71e99b265fb49986acb7fb99206` is unchanged in release `3b25931ccd7cf0fd2dc3237bfc7f73c552e55260` (tree `df386219b2aaec9c7d90a3f2e88bce618a210be1`), tagged once as `v0.1.16`. Its three implementation/test files retain their reviewed bytes: production diff SHA256 `eed5cbdb17226f23f47a8c571392a5ee332bb3f9295d6ee9d123ed5c1d9c8a46`, new-test diff `402c7bb1f8f72248320e19b83e61cc843e36ac905ec0b4f6c9403715c838bc4a`. The shell/checks remain 2,034/528, estate/pipeline tooling 5,631 + 2,330 = 7,961; no scope law, census, dependency, pin, resource, interface, configuration, state format or known defect changed. Candidate B remains independent, unscheduled and unaccepted. This record changes only completion facts; it is not the published source.

**Artifacts and workflow gates.** Exact-release PR CI [34554632034](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34554632034) passed all four jobs; its synthetic merge `59345d7834d56c8fc8d7dfaded3eba90fb2f67f8` has the release tree. The real TestPyPI rehearsal [34555173141](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34555173141) passed build/TestPyPI and intentionally skipped PyPI; the tag workflow [34556181309](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34556181309) passed build/TestPyPI/PyPI. Both public indexes contain only the intended wheel, SHA256 `7eba96f16eb749638aa48ad70111d92469999b1bd44918fbcc5162c24cd018e3` (188,580 bytes), identical to local and workflow builds. The rehearsal uploaded TestPyPI at 02:35:46.064866Z; the tag workflow correctly kept that existing identical wheel and uploaded PyPI at 02:52:06.508058Z. Local sdist `14b2fdac0bf7ef87936dcc42924a057c49a8061efa6e38374d58a56839ad2e7a` (191,378 bytes) was validated and never uploaded. Both archives have 18 entries; the wheel contains all seven required paths, source/resource mappings and exclusions pass, and sdist-built and directly built wheels agree. The retained 0.1.15 preparation wheel was not published.

**Matching image and order.** One build/push from public `v0.1.16` plus the actual PyPI wheel produced `ghcr.io/mhganainy/gsj-polar:f0e8343a-gsj0.1.16`: OCI index `sha256:1ae10e8c23c280c2e04e0d80619c209cce0b2ef93c71edec7c6eeaebe45732ec`, linux/amd64 manifest `sha256:68bdc65fd9280fa845aaa8756fce6e0a24c07036a14aa9898fd4dc0985eff36f`, linux/arm64 manifest `sha256:d5942f974501a70726470a55592428f09a043266218e48abb5d36e3fe3a174d1`. Both anonymous pulls and executions passed version/source, all 13 wheel payload/resource mappings, patched Polar harness/builder inheritance and required behavioral seams. The amd64 execution was emulated on the arm64 Docker host, not a second native host. Build/push ended 02:59:23.881641Z; first anonymous index response 03:00:30.288112Z (503.780054 seconds after PyPI), first anonymous child pull 03:00:34.535756Z (508.027698 seconds), both platform execution proofs complete 03:02:41.896632Z, final identity/cleanup gate 03:02:42.320331Z (635.812273 seconds). A first probe misparsed the existing `POLAR_SHA=` record token; the proof harness was corrected against the actual record and its full hash, then the same immutable image passed. No image was rebuilt or republished to hide the failed probe. Only after these gates did main fast-forward to the release commit at 03:05:14.072972Z; [PR #2](https://github.com/MHGanainy/gsj-harness-rollout-server/pull/2) closed merged at 03:05:15Z, with no force push, fabricated approval or bypass. Exact-main CI [34557101890](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34557101890) passed all four jobs. MCP 0.5.0, Forgejo, pi, Polar and its patches were not changed or recut.

**New release proofs, with their limits.** Local source suites passed 176 root, 541 corpus, 164 MCP and the required 25 Polar patch checks. Both public indexes were installed by name into fresh neutral environments; project downloads, installed paths, metadata and bytes identify the published wheel. TestPyPI used the single project-index arrangement with ordinary dependencies installed separately. Each index passed all seventeen installation checks, installed CP-92/94/104 seams (29/21/35), and the applicable row-111 proof (24 before the image existed on TestPyPI, 25 after it existed on PyPI). The initial TestPyPI guide comparison encountered the intentionally old main under ADR-0043; the passed installation/seam results were retained, and strict public-main guide checks completed on both indexes after main advanced. Check 13 deliberately selects 28 tests and deselects 28; this is not a skip of a required gate. Hosted corpus CI passes 540 and skips the one local demo-thirty fixture; the full local 541 passed. The additional host-live proof and both fresh nested runs are PyPI-only, not new both-index claims. The nested whole-trajectory no-TMPDIR arm produced ERROR with exactly `ADM1:status_not_completed:ERROR`, one trace, a fully reconstructed chain and the observed transcript-download failure; restoring the documented TMPDIR line collected a COMPLETED body independently validated to `[]` (17 characterization assertions). The separate README routes passed 22 assertions: neutral trainer-only and estate-only submits exit 3, then an actual anonymous new-image pull and the actual public-main checkout route each collect accepted bodies independently validated to `[]`. Installed missing-runs refusal plus real nested mkdir/up success is the established coverage; no additional same-run nested refusal/cure pair is required. Stock-Ubuntu venv/PEP668 and the full upstream Polar failure observations remain historical; conditional re-vendor/pin/wire canaries and H200/production execution were not triggered by this release. Row 24's production-owned obligation remains at its existing boundary and is not waived by these isolated proofs.

**Demo and closure.** The existing demo changed only release floors/references, their literal assertion, front-door release prose and a factual appendix to its existing image ADR: commit `0629cda5a5dd60a50c7f4793d2505281d2a1ba38`, tree `2a654eddf63544851d6b8b914d9071a48fc9532a`, parent `34d9d8c04e531da7ac284d947b0516d8eb21934c`. All 89 tests passed. A fresh local clone of that exact committed tree, before push, installed the public PyPI wheel and ran A-28/step 1b on the proven image: t=4 session `sk-polar-3063bc62-edb5-4b36-bbd3-e02c72dc72fc` actually searched pages 1–4 and named deed 98-4417 on page 4, registered 11 August 1998; t=2 session `sk-polar-09370baf-ade0-49ec-a94a-0a1088f2187c` actually searched only pages 1–2 and named no page-4 deed number. Both were read back/exported and independently validated to `[]`. **The t=2 answer nevertheless hallucinated a page-1 deed and date; this is a task-quality limitation, not a truthful negative or a training result.** No retry, prompt change or product fix was used to obtain the passing cutoff/provenance result. `down` exited 0 and restored the captured original Docker inventory. The exact tested demo commit was pushed normally at 03:22:22.926703Z and independently read from its remote at 03:22:23.993466Z. A-28 is fulfilled for 0.1.16; its next-release duty remains. All isolated proof containers/owned volumes were cleaned up with inventory comparisons; original estates, historical evidence, existing worktrees, examples and archived predecessor were preserved. Raw command/provenance receipts, public artifacts, protected session bodies, manifests and the normal CP-106 report remain local-only under the Candidate A worktree's ignored `docs/reports/`; the comprehensive `docs/refactoring/refactoring-review-handoff.md` remains ordinary untracked content. The later completion-record commit, push and its CI are recorded there after execution. No unrelated production deployment or governance amendment occurred.

**[Release 0.1.17 / PR #3] Obligations recorded before publication.** The reviewed functional commit is `1105a44d08380c0c659a5c951f40c0f6ccc8906f`, based on `482b04140d8511498e162c2fed72077ccd8ba378`; its ten-file patch SHA256 is `f4f5a504f9a8cd345a7d0d45709e9522bf32899be0f56230eb1a19fda21ee596`. It groups created-retrieval activation without changing checkpoints, shares generated-JSON reading, and uses Starlette Response with unchanged encoded bytes and HTTP emission. Root 0.1.17 and retrieval 0.5.1 are independently versioned patch releases. The service-local Starlette 1.6.0 declaration does not change trainer/corpus dependencies. Release changes advance fresh-run retrieval defaults and matching Polar references; persisted runs retain recorded selections and adopted services remain externally owned. The shell/checks equalities stay 2,034/528; tooling is 5,654 + 2,324 = 7,978, a clarity/shared-behavior change rather than a code-reduction claim. Packaging B, the proposed census ADR, native progress and known defects are excluded.

The release source stays on PR #3's branch while its exact tree passes CI and local artifact checks. Both retrieval platforms must be built, measured and published before a public wheel selects them. The existing TestPyPI rehearsal and tagged wheel-only TestPyPI/PyPI workflow then precede Polar's public-tag/PyPI-wheel build. A-28/ADR-0043 require anonymous platform pulls and runtime provenance before main/front-door/demo advance. Fresh named installations from both indexes, installed checks and seams, applicable nested failure/cure and README routes, and the exact committed demo's suite and t=4/t=2 read-back proofs remain required. Earlier applicable evidence stays historical; answer truthfulness is separate from cutoff/provenance. First-production-bring-up row 24 remains open, with no production deployment or new pin/wire/H200 trigger implied by publication. ~~`waiting-on: release 0.1.17 completion record after publication, both images, main, demo and required proofs succeed`~~ (fulfilled by the completion record below; the source/tag stays separate from that later record).

**[Release 0.1.17 / PR #3 — completion record.]** The reviewed functional commit `1105a44d08380c0c659a5c951f40c0f6ccc8906f` shipped with release metadata at `0bc15de7f034d6f031b4135d1fe0245ec31ddb52`, tagged once as `v0.1.17` (annotated object `e87afdc29daffe8cb41b26495cb324b512482a4a`). PR [#3](https://github.com/MHGanainy/gsj-harness-rollout-server/pull/3) was made ready and normally merged at `08421222b7d37d9f14792c156730c66580ca2bfc`, 2026-09-11 21:25:04 UTC; its tree `bdb8181d566e42176ac1c4189d1a6ac77f24f56e` equals the release tree. Main stayed at `482b041` until both published images passed their required proofs. No branch rule or external approval was bypassed. Functional scope remains the three reviewed refinements: created-retrieval activation, shared generated-JSON reading, and Starlette response emission. Release-only changes advance version/image references and independently derived metadata expectations. Tooling is 5,654 + 2,324 = 7,978 lines (+17); the shell remains 2,034 including checks.py at 528. This is clarity and shared behavior, not a large reduction. No native-progress change, pin/model/vendor/schema/persistence change, packaging relocation or governance decision is included.

**Root artifact and CI.** Both public indexes serve one identical 189,000-byte wheel, SHA256 `93d4cd92bf303bd09b7a1e16ab5f3479c9b02d75ab0c14e53fba3d37f8b7f87d`. The local direct wheel, sdist-built wheel, hosted artifacts and named public downloads agree. Both archives retain 18 members, with all 13 Python/resource mappings exact; dependency metadata and unchanged resources match 0.1.16. The sdist is verified locally and unpublished. Local root 176 and corpus 600 pass; lightweight and PyYAML-only source invocation pass. Final source CI [34638453554](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34638453554) and actual main CI [34649278977](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34649278977) pass all four jobs: root 176, corpus 599 plus the existing missing adjacent demo fixture skip, MCP 256, and installed wheel/pins proof. CI's historical display labels 541/164 remain distinct from actual counts. Rehearsal [34646384446](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34646384446) passes build/TestPyPI and intentionally skips PyPI; tagged [34647589291](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34647589291) passes all three jobs. Initial public-index propagation misses are retained. The tag workflow's TestPyPI log says Uploading/HTTP200; independent before/after URL, hashes, size and original upload time prove the already published identical artifact was retained.

**Retrieval is delivered separately.** `ghcr.io/mhganainy/gsj-mcp-service:0.5.1` index `5602a693fe86cc52ef2df3ca590e1b56c149d7f5c2d6998f20b54bc8f1b697ca` contains amd64 manifest `5c7dd78afa7b7f5728998783a5b4a1f34347bdd7dcc1613af5df79d9e023c61a` and arm64 `c4cc0da50ae21865b402b7f98129544aeba5eb0b75d9e7a5fc23ab092aedc93a`. Actual loaded app SHA256 is `b4123cdbb0ac457784eb8aae7909ce6b3697006a8e95eed80cecdc03aed039eb`, its helper binds Starlette Response, and installed Starlette is 1.6.0. Anonymous actual-image checks match 154 raw ASGI observations on each platform. Native ARM real Git/Chroma/model/MCP/admin integration matches the independent actual 0.5.0 control, including replies, ranking and index reuse. No trainer/corpus Starlette dependency was added.

Unconstrained rebuilds showed incidental transitive/OS drift and were withheld. This release instead used the existing `estate/mcp-service` context with a recorded four-line recipe: `FROM ghcr.io/mhganainy/gsj-mcp-service@sha256:17704c06d703ab887c045cdd621088916c5594c258ac2ddc1f34332f702c1f6d`, then `COPY requirements.txt ./`, `COPY gsj_mcp_service ./gsj_mcp_service`, and `COPY config.yaml ./config.yaml`. Recipe SHA256 is `ad119c5d48d08bb54954c7036fd0e180f398aeee97559f4c349388b4c5206dd5`; explicit version/revision/source labels identify 0.5.1/0bc15de. The old twelve immutable layers plus three COPY layers preserve actual 108 Python distributions, 118 OS packages, runtime configuration apart from those release labels, and model bytes. The ordinary source-build Dockerfile is unchanged apart from version examples; a future unconstrained rebuild is not promised to reproduce this frozen closure. Both platform builds, anonymous pulls, actual source and closure proofs pass. Extra full-backend AMD emulation reproduces the old image's signal 11 and is not counted as a pass; VERDICT row 49 already records that failure class and native ARM selection as its cure. No native AMD model-backend/H200 result is claimed. Initial observer timeouts, failed alternative builds and two image-proof representation assumptions are preserved with their corrections; no production timeout or parsing behavior changed.

**Image before front door.** Matching Polar `ghcr.io/mhganainy/gsj-polar:f0e8343a-gsj0.1.17` was built by the existing demo recipe from public `v0.1.17` and its actual PyPI wheel. Index `6a385c03445a97f7dd046d177af1b3944c84cd1e613b911a1988bfd8560e9c29` contains amd64 `408fba68f58330945881b5b5dec6de91501aa23bac9ee80970166702cfc20101` and arm64 `9ca9b51fd3b4a8de9a1c967d463ee6074381edcceeaa1d87447a89eb4913ff47`. Both anonymous pulls execute the exact source/wheel payloads; 21 Python versions, 118 OS packages, binaries and configuration match 0.1.16 except GSJ 16→17. ARM executes natively in the Docker VM; AMD executes under emulation. PyPI upload 21:06:21.594501UTC → first observed anonymous index200 at 21:18:40.727314UTC is 739.133 seconds; both runtime proofs finish after 756.958 seconds. Main advances only afterwards. These are observation bounds; the residual wheel/PyPI-page window is the one ADR-0043 already names.

**Public consumer gates.** Fresh named installs from TestPyPI and PyPI pass all 17 standing checks plus archive 0 and installed seams: CP90 23, CP92 29, CP94 21, CP104 35; check13 has 28 selected passes and 28 deliberate deselections (overlapping groups are not summed). Both strict public-main guide checks pass. TestPyPI row 111-absent (24 checks) passes before image publication; PyPI row 111-present (25 checks) passes afterwards. Published retrieval 0.5.1 actually serves 313 decision units in the fresh runs, with app/Starlette/selected-native-image assertions. Credential refusal/cure, adoption and adopted-only down preserve parent ownership. The host live episode is COMPLETED with 239/239 trainable positions and 9 tool calls, independent findings `[]`, matching receiver/trainer bytes and no run secret. Reaper/occupied-port/exec127 and provenance cases pass. Nested failure/cure passes 18 checks: no-TMPDIR returns whole ERROR and exactly ADM1 on independent raw-body validation, then the documented cure returns COMPLETED/`[]`. README routes pass 22: trainer-only and estate-only exit 3; actual anonymous image and public-main checkout episodes each validate independently to `[]`. Both nested gateways' secret lengths and node registration are observed before episodes. This does not close first-production row 24.

**Recovery and preservation, including the failed guards.** A real 0.1.16 run was interrupted after retrieval readiness and before acknowledgement: exit 130, persisted compose selection/rebuild-always, no top-level success or taskbank, and matching partial status. Public 0.1.17 resumed successfully with recorded retrieval 0.5.0. The first outer proof failed because it wrongly required an unchanged container ID: both old source and a new independent old→old live control require `compose up -d --force-recreate mcp` for that ready-but-unacknowledged rebuild. The control verified recreation with unchanged image/spec and reset only after success; baseline warm and public warm both omit force-recreate and preserve the same post-rebuild container, old image/native selection and successful status/down. No product correction was needed. Partial stdout is exact; raw stderr differs only by the independently installed interpreter prefixes, retained without gate normalization.

The second outer wrapper failed its whole-daemon equality because a concurrent foreign BuildKit container/state volume appeared. Read-only process evidence ties active use to a different session/workspace; the original creator was not directly observed. All pre-existing resources remained unchanged and all owned recovery resources were removed. Neither failed wrapper is relabelled green; the observed recovery/preservation contracts are demonstrated separately. No foreign builder was removed and no rerun was used to hide the concurrency. Host and nested gates restore their complete captured inventories. Original worktrees, the original `.gitignore` modification, older reports, deployment state, examples and archived predecessor are preserved; only owned proof resources were cleaned.

**Demo and closure.** Demo commit `aefe16b4e363a4892676bbd44b64f322eb5f3ad2`, tree `14fe1c740f45d5dec6410cd6d5af878c6b6ab280`, changes only six release metadata/documentation/expectation files from `0629cda`. All 89 tests pass with the README's named public PyPI install. A fresh clone of that exact committed tree passed step1b/t=4 and t=2, read/export and independent validation to `[]`, then pushed normally to demo main at 2026-09-11 22:40:30.917924UTC. t=4 session `sk-polar-ca78d797-9817-406d-8cd7-c329e087e0e1` searched pages 1–4 and correctly named page 4/deed 98-4417/11 August 1998. t=2 session `sk-polar-527b4ade-7e58-4c9d-86a8-242385819619` searched only pages 1–2 and named no page 4 deed number, but **hallucinated an easement deed on page 1 and guessed 2019 registration**. This new task-quality failure is distinct from the passing cutoff/provenance gate; the earlier hallucination is not erased. No prompt/model retry or product correction was used. Down exited 0 and restored the exact full inventory, including the pre-existing foreign builder. Remote demo main/tree match the tested commit; its API lists zero hosted workflows, not a passing hosted suite.

A-28/ADR-0043 delivery is fulfilled for 0.1.17; the next published payload change repeats the standing duty. The register scan found 103 matching `waiting-on:` lines before this completion record and new handoff; the 0.1.17 delivery token is closed by this record, while first-production row 24 and clone-storage row 47 remain outside this release. Native H200/golden/wire recollection was not triggered by unchanged vendor/pins/episode models; no such pass is claimed. Packaging B and the proposed census ADR remain deferred. The new ordinary untracked release report carries full source/record/demo diffs, exact artifact and runtime identities, commands, failures and corrections, retained versus new results, resource accounting, and this later record's commit/push/CI. The immutable source/tag remains 0bc15de, distinct from this completion record. No production deployment occurred.

**Native Docker progress release 0.1.18 — obligations recorded before publication (2026-09-12).** The owner approved and published for review functional commit `0521599bca09e08572b062f152159594950bc523` on PR #4, complete thirteen-file patch SHA256 `1b14cf9bc091f1388f574e827bda017b53538450b2a1ad60e878d59fb29daad7`. Docker inherits stdout directly; custom host-byte sampling, layer parsing/tally aging, pull heartbeats and stdout forwarding are removed. Redirected output may remain quiet during a large transfer. After ordinary `proc.wait()` returns, the existing whole-read stderr reader receives up to ten seconds to finish, returning sooner when it finishes; there is no separate inherited-stdout wait. The approved descendant-pipe timing change is not a Docker execution timeout or partial-stderr guarantee. Existing decoding/read suppression, caller diagnostics, cached/loaded/local image policy, readiness, locks, activation and recovery remain. Estate 5,496 plus corpus 2,324 totals 7,820 tooling lines (−158 production); shell/checks equalities remain 2,034/528. Root 0.1.18 was selected after Git, PyPI and TestPyPI absence checks; retrieval stays 0.5.1, with its source, image, dependencies and model closure unchanged. Functional bytes and behavioral expectations are retained; only root version, current image/install references, necessary version expectations and factual release records change. Shared-decisions, Chroma cleanup, packaging changes, pins/models/prompts and deferred defects remain independent. No checkpoint or accepted ADR is minted or amended.

The established release gates remain: final-source four-job CI, local direct/sdist-derived wheel and resource/metadata checks, lightweight trainer and PyYAML-only source proofs, TestPyPI rehearsal before the tag, fresh named installs from both public indexes with all seventeen standing checks and installed seams, the changed native-pull boundary driven through installed release artifacts, and applicable live/printed-command/recovery/nested/consumer proofs. Historical source-only results are reused only by byte identity; they are not called fresh published-artifact passes. The optional admin fixture remains historical unless explicitly remeasured. Controlled subprocess tests do not establish cancellation of an in-flight daemon pull, and the previously disclosed foreign BuildKit inventory-guard failure and demo hallucination remain recorded. A-28 and accepted ADR-0043 require the matching Polar image from public source and the PyPI wheel, preserved dependency closure, both platforms and anonymous child pulls before normal merge to main or demo reference advancement. A later completion entry records actual source/tag/merge/main/demo identities, artifact hashes, outcomes, failures, cleanup and the observed wheel-to-image window. ~~`waiting-on: native-progress 0.1.18 completion record after publication, matching image, normal main merge, demo delivery and required proofs succeed`~~ (fulfilled by the completion record below; immutable release source/tag remains separate from this later record).

**[Release 0.1.18 / PR #4 — completion record.]** Reviewed native-progress commit `0521599bca09e08572b062f152159594950bc523` shipped with separate release bookkeeping at `989060c8fe4c924152337797b6a93e79e389b72a`, tagged once as `v0.1.18` (annotated object `b5256891a6a2437b37c8ccc56a7df661e5bd63a5`). [PR #4](https://github.com/MHGanainy/gsj-harness-rollout-server/pull/4) was made ready and normally merged at `5b75f377818886a72e8d82cc3f0cc95b582ee263`, 2026-09-12 11:09:17 UTC. Its tree `fd1c4e058e7af88b4bf81e8c2da3a060019e5cc5` equals the release source; parents are prior main `25651eb25ba7f4e3d787c01a9c75f543b8de4b55` and release source. No intervening main change or unreviewed scope was absorbed. All source CI checks passed; there were no outstanding reviews, protected-main requirement or effective main rule. The owner-authorized normal merge used the exact head guard without force, administrator bypass or auto-merge. Main stayed on the previous release until the matching image's anonymous platform, runtime and installed native tests passed.

Docker inherits stdout directly; custom pull telemetry, layer parsing/tally aging, heartbeat rendering and stdout forwarding are removed. Redirected transfers may be quiet. After ordinary `proc.wait()` returns, the existing whole-read stderr reader gets up to ten seconds to finish, returning sooner on completion; stdout has no separate wait. The approved descendant-pipe timing change is neither a Docker execution timeout nor a partial-stderr guarantee. Existing decoding/read suppression, caller diagnostics, image policy, readiness, locks, activation and recovery remain. Production tooling is 5,496 + 2,324 = 7,820 lines (−158); shell/checks remain 2,034/528. Retrieval 0.5.1 and its existing source/image remain unchanged. Shared-decisions, Chroma cleanup, deferred defects, packaging proposals, dependencies, vendor/Polar patches, pi, Forgejo, models, pins, prompts, schemas and persisted formats remain independent.

**Root artifacts and CI.** Both public indexes serve the same 186,874-byte wheel, SHA256 `995013873f545a3127e0bccdc4e3e8ebe8286554264aa81fed66a8b9693d3b00`. Local direct and sdist-derived wheels, both hosted builds and actual named public downloads are byte-identical. Archive 18 members, all 13 source/resource mappings, RECORD/metadata/pins and installed provenance pass; actual 0.1.17 comparison changes only __init__/estate/METADATA (including the README body)/RECORD, preserving all other payloads and dependency declarations. Lightweight trainer and standalone PyYAML-only use, installed CLI/recovery seams and preserved dependency closure pass. The locally checked sdist remains unpublished; existing 0.1.17 artifacts/tag are preserved. Source CI [34683568009](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34683568009) checks synthetic `b2a85f3bb8c0eda85cf6524f6bda05cc2ba3b61a` with the exact release tree: root 176 passed, corpus 624 passed/4 skipped, retrieval 256 passed and packaged wheel/pins proof passed. Quiet logs omit individual skip reasons, so the adjacent-demo/two ARM Forgejo/optional-admin attribution remains source-derived inference. Actual merged-main CI [34690298210](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34690298210) passes all four jobs on exact merge 5b75f377 / tree fd1c: root 176 passed/zero skips (23.89s), corpus 624 passed/4 skipped (133.20s), retrieval 256 passed/zero skips (390.35s), and packaged wheel/pins PROOF OK. No source correction or CI rerun occurred; transient read-only TLS monitoring failures are retained separately. Historical job display labels 541/164 remain separate from actual counts. Rehearsal [34683950031](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34683950031) passes build/TestPyPI with intentional PyPI skip; tagged publication [34685610834](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34685610834) passes all three jobs. Initial index-propagation misses and identical-command successful retries are retained.

**Image before front door.** Matching `ghcr.io/mhganainy/gsj-polar:f0e8343a-gsj0.1.18` was built by the existing demo recipe from public `v0.1.18` and the actual PyPI wheel. Index `0c390d419f0c1403ecb79adbb24bfa75a6f2e7fa6c04b177d6d49bb7161bbe71` contains AMD manifest `52c608d612a28c1865c48500a57d062e11b0084e6e0d967a9fa1094f519a70bd` / config `29df3d7e8b8261e5cac4864a0f36cb0428d9c66126170fc845bf1b5e89fd3117`, and ARM manifest `e86c87ab75d23d380347bc06dcc009bfb60e69e78fea4cf109848c70252e5c11` / config `b34aa65a1d0b7ee8fa336e228158b8c837de736f643014089cab17ca6a4163bc`. Operationally pinned FROM indexes and an ordinary pip-constraints BuildKit secret preserve all 21 runtime distributions apart from root 0.1.17→0.1.18, all 118 OS packages, Python/Docker CLI binaries, runtime configuration and all 91 installed Polar files. No runtime constraints file or new dependency framework ships. Both anonymous actual-image pulls and installed 13-payload/source proofs pass; local and public index/config bytes agree. The actual loaded image ID is recorded independently: local containerd projected IDs are child manifests, whereas hosted classic Docker reports the AMD config ID.

PyPI upload 09:22:01.534702UTC → normal image push 10:26:18.458382UTC → first retained anonymous index 200 at 10:26:55.721703UTC is 3894.187001 seconds to that observation. Both anonymous pulls finished 10:27:27.485762UTC and runtime proofs 10:27:34.738481UTC. These are observation bounds on ADR-0043's existing wheel-to-image window, not the first instant of global availability. Native ARM installed lifecycle 23 and callers 11 pass, zero skips/errors/failures; a shallow observer-output mount first prevented caller collection, then a mount-only correction passed the callers while reusing the successful lifecycle identity. Under AMD emulation the original aggregate observer timed out; a larger aggregate observer budget then completed 23 lifecycle cases with 16 passes/seven timing failures. No test/worker assertion or deadline was relaxed. Fresh native GitHub X64 [34690077750](https://github.com/MHGanainy/gsj-harness-rollout-server/actions/runs/34690077750) passed the original 23 lifecycle/11 caller tests with unchanged 180-second groups, four deliberate caller deselections and zero skips/errors/failures. Its operational workflow commits `fd18ad252170365bb42a6e70b277463a24abc4bd` and `b57e38c8a25fbbb28d8d5e558449cefab8a3dc03` are retained on a separate branch and never merged into release/main; the first was rejected with zero jobs/tests for an unavailable YAML context, corrected only by shell path initialization. Native proof checks out source 989060c8 explicitly. No native H200/GPU coverage is claimed; the emulated failures remain failures.

**Publication failures preserved.** The first constrained AMD image build failed resolving compatible PyArrow 25.0.1; independent index/interpreter probes and an unchanged retry succeeded without dependency changes. Two normal image pushes failed with EOF at one exact compressed layer. A rejected ranged-upload attempt and an empty-finalization PUT with the observer's wrong content type are retained. The exact tested blob was acknowledged using the established streaming client convention, finalized on that same acknowledged session with the corrected content type, anonymously verified and followed by a successful normal push of the identical index. No image rebuild, source correction or assertion change hid these failures. Zero-byte permission-preflight and rejected/unfinalized registry sessions have unavailable status/abort or unmeasured expiry limits; no claim of their deletion is made. Sensitive signed upload evidence remains private. Read-only TLS/DNS failures and the obsolete Projects-classic PR-edit error are retained with scoped retries/REST correction. No product fix was required.

**Public installations and recovery.** Fresh neutral TestPyPI and PyPI named installs each pass all 17 standing checks plus archive 0, public module/resource origins and installed seams; relevant retained CP92/94/104 groups are 29/16/32 passed, with explicit overlap/deselection attribution in the release report. Both public artifacts independently pass 23 lifecycle and 11 selected callers (four deliberate deselections, no runtime skips). Both strict current-main guide checks passed after the normal merge, freshly matching public anchors/source bytes while explicitly reusing unchanged 29-case regressions. Optional admin support remains historical. TestPyPI's pre-image row 111 proof passes all 23 behavioral assertions but both its final Z global-inventory assertion and outer inventory guard remain failed after foreign additions; independent audits prove preservation of all pre-existing entries and removal of every owned resource. PyPI's image-present row 111 proof passes all 25 checks including both global guards: actual printed commands collect an independently validated `[]` episode, and the cold redirected pull emits Docker's native stdout without custom telemetry. The slow 366.6-second pull remained within its unchanged 1200-second observer budget; that budget is not a Docker execution timeout. The actual PyPI host live proof passes: reaper bound 6.1s, owned live removal 0.6s, occupied port correctly UNMEASURED, owned exec 127 correctly reported as failure, and preserved recorded EOS provenance labels. Its episode completes with 688 trainable of 3,449 total positions, 2 tools and stop, independent findings `[]`, and identical receiver/trainer bytes. Full inventory and dependency freeze remain unchanged. A small cold terminal pull through the actual installed public helper verifies isatty, native Downloading/Pull complete/cursor output and successful completion (4.175s, 2,965 bytes). Its first HTTP-only loopback fixture produced a preserved TLS diagnostic; correcting only the explicit 127.0.0.1 endpoint/HTTP handshake used Docker's existing fallback. No daemon configuration/source/dependency change occurred, existing insecure CIDRs and full inventory stayed exact, and the owned fixture/image were removed.

Actual published 0.1.16 was interrupted after retrieval 0.5.0 readiness but before acknowledgment (expected 130); actual PyPI 0.1.18 resumed the incomplete recorded run using its original image/Compose selection while its fresh default remains 0.5.1. First resume correctly recreated the unacknowledged rebuild container, acknowledged only after success, and warm resume retained the new container without force-recreate. Status/down pass. Partial stdout is exact; raw stderr differs only in full venv paths, retained as false before separately documented prefix normalization. All 60 original observer AST assertions remain, not 60 independent tests; the earlier old→old control is reused. The full 14-container/16-volume/3-network inventory is restored. Historical foreign BuildKit-size/global-inventory failures remain disclosed; owned cleanup passed. Controlled subprocesses do not establish cancellation of a daemon's in-flight pull.

**Nested routes and demo.** The first nested pair stopped during installation: the fixed PyArrow 25.0.1 ARM wheel download stalled after 1.1 MB and pip returned 2 with a files.pythonhosted.org ReadTimeoutError. The actual GSJ wheel had downloaded, but no installed-provenance or episode gate had run. Original pip 120s / 5 retries, install 1200s and characterization 3600s budgets were unchanged; the whole observer budgets did not expire. The failed command and raw logs remain, and owned cleanup restored the entire captured inventory. The unchanged fresh pair retry passes all 18 checks. The missing-TMPDIR case returns whole ERROR with exactly ADM1 and 217 trainable positions on independent raw-body validation; the documented cure returns COMPLETED with 536 trainable positions and independent findings `[]`. Actual public wheel, installed/image source, constrained dependency closure and both gateway registrations are verified; full owned/global inventory cleanup passes. The first documented-routes attempt stopped at the trainer installation with pip ResolutionImpossible: Pydantic required core 2.46.5 and the constraint required the same version. Both PyPI APIs subsequently listed the compatible non-yanked ARM wheel with the exact hash installed by the successful pair; the declaration and constraint agree. Failed logs did not record candidate-discovery details, so the exact cause remains unestablished rather than called a proven version conflict. No route behavior is credited to that attempt; all owned/global cleanup passed. The logging-only retry passed 16 recorded checks and the actual published-image episode (COMPLETED, independent `[]`, 253 trainable positions), then stopped before the checkout episode because the new dependency observer rejected duplicate Polar distribution metadata. Read-only reproduction identified the established editable dist-info/egg-info pair; their metadata, entry points and top-level bytes agree and their editable origin is the same vendored project. The corrected observer accepts only that exact expected-version Polar pair after verifying all those identities, still rejects other/third duplicates, records every metadata origin before assertions and retains complete dependency equality. An intervening fresh attempt hit the unchanged 60-second GitHub TLS handshake limit before any Docker creation; no behavior is credited to it. The final identical retry, started 13:24:36.483274 UTC, passes all 22 route checks and exact 13/14/20/19 trainer/pair/checkout/Polar dependency profiles. Trainer-only and estate-only submits retain the expected exit 3. Anonymous-image and fresh public-main checkout submissions both complete with independent findings `[]`, one trace each, and 195/491 trainable positions. The checkout is exact merge 5b75f377/tree fd1c4e; the actual public wheel and installed module/resource identities agree. Fresh Polar metadata measurement confirms the exact editable alias, with unchanged metadata SHA256 `6d6425283a2bc43857236d4acaa1176ba56442dd8293a326891346831081e7d6`. Every original behavioral assertion, fence, dependency constraint and deadline remains; only private observer logging/identity handling changed. Inner and outer cleanup restore all 14 containers, 16 volumes and 3 networks. These are native ARM nested proofs, without H200 or model-correctness claims. Demo commit `e2584036d24e2b4e5f991594bc427d64d33b2b4e`, tree `ced3915e2de3fe237803dd23cf62f6c77d83e71c`, changes only six reviewed release metadata/documentation/expectation files from `aefe16b4e363a4892676bbd44b64f322eb5f3ad2`; every earlier ADR byte remains an exact prefix. Its exact-commit suite passes 89 tests with zero failures, errors or skips. A fresh-clone proof against the actual public 0.1.18 wheel and matching Polar image passes all 12 retained checks (12:11:42–12:21:05 UTC), including two accepted episodes with independent archive findings `[]` and exact owned/global cleanup. The t=4 answer correctly names page 4 and deed 98-4417, but uses a natural-language citation rather than the literal `(page:N)` format. The t=2 answer searched only pages 1/2 and omitted 98-4417, satisfying the retained cutoff assertions, yet hallucinated a page-1 easement deed and 2019 registration; overall answer correctness does not pass. No prompt/model change or preferred-answer retry occurred. A pre-push read-only patch guard first failed because abbreviated Git index headers differed from the reviewed `--full-index` serialization; the observer-only correction proved exact reviewed patch bytes before the single normal `git push origin HEAD:main`. That push completed 2026-09-12 12:26:29.277321 UTC to `https://github.com/MHGanainy/gsj-rollout-demo.git`, and a fresh remote read confirmed `main` at `e2584036d24e2b4e5f991594bc427d64d33b2b4e`. The exact commit contains no hosted workflow files and the repository Actions API lists zero workflows, so no demo hosted CI is claimed. Admission/cutoff/provenance acceptance remains separate from answer correctness; earlier hallucinations are not erased. Trainer examples remain at their existing compatible floor, with no affected runtime/Polar reference requiring an update. The post-proof preservation audit verifies all 2,645 captured files unchanged: both prior native-progress reports, 2,629 original-checkout files including its existing .gitignore modification, and 14 earlier blocked-candidate files. All historical worktrees remain, including the independent shared-decisions and Chroma investigations; original checkout and demo heads remain preserved. All owned runtime containers are removed; image tags, caches, private environments and evidence are intentionally retained. Earlier foreign-inventory changes and unmeasured registry-session expiry are disclosed separately from successful owned cleanup.

**Closure and remaining duties.** A-28/ADR-0043 delivery is fulfilled for this payload; the next published payload change repeats the standing duty. At 2026-09-12T13:38:45.132226+00:00, `rg --no-ignore --hidden -n 'waiting-on:' docs/` in `/Users/elganayni/mg/workspace/gsj-native-progress-stderr-20260912T021622Z` returned 109 matching lines / 179 literal occurrences across all searchable docs, including ignored/untracked reports. This records the pre-record scan; historical struck tokens and literal instructions are included, so it is not an open-duty count. Only the native-progress completion token above closes. First-production row 24 and clone-storage row 47 remain open; no new checkpoint, scope-law/accepted-ADR amendment, production deployment, pin/model/vendor update or H200/golden/wire recollection is claimed. Shared-decisions and Chroma investigations, deferred fixes and packaging/census proposals remain independent. The ordinary untracked release report `/Users/elganayni/mg/workspace/gsj-native-progress-stderr-20260912T021622Z/docs/refactoring/20260912T082355Z_native-docker-progress-release.md` carries complete functional/release/demo/operational/later-record diffs, actual public artifact identities, commands, retained failures, new versus reused evidence and this later record's own commit/push/CI. Immutable release source/tag, normal merge and this later factual record remain distinct.
...
10. **A fake's shape is measured, never written** — when a test fakes a
   CLI (Docker, Compose, git), the fake's exit code and its stdout and
   stderr are part of the contract the code under test is written
   against, and something in the suite must measure the real one. Added
   at CP-98 on the second occurrence of the class: CP-84's Compose
   grammar (a fake that did not speak Compose's real output, caught only
   because that checkpoint ran real Compose), then row 100 — CP-96's
   reaper faked `docker rm -f <missing>` as exit 1 where the real CLI
   exits 0 with the message on stderr, so the bound the test proved never
   ran on any daemon, and it shipped green in 0.1.12. A fake written from
   memory proves the author's model of the CLI, not the CLI. Mechanism:
   **(a)** `estate/corpus/tests/cli_shapes.json` records every shape a
   test fakes and the code branches on — exit code, stdout, stderr, what
   reads it, and the CLI and daemons it was measured on; **(b)**
   `test_estate_cp98_reaper.py::test_cli_shapes_match_the_real_cli`
   re-measures every entry against the CLI on PATH wherever a daemon
   answers (the workstation, the CI runner) and skips, naming the reason,
   where none does (a firewalled daemon with no `alpine` — the H200's
   shape); **(c)** a fake takes its shape from that file through
   `conftest.cli_shape` — a shape the file lacks is measured and added
   before the fake is written, never guessed; **(d)** a proof from an
   installed artifact that drives the real CLI measures what it drove and
   nothing else — CP-97's proof measured `rm -f` on a live container and
   on a missing name (which is how row 100 was found), not the create
   race. The file's own first draft at CP-98 wrote one shape wrong
   (`docker inspect --format … <missing>` prints a bare newline on
   stdout) and (b) refused it on both daemons before it was committed —
   the rule's first catch was of the rule's author. Retrofit at CP-98:
   the reaper's fakes, the `docker inspect` fakes, the `docker info`
   fake that still answered the pre-CP-96 one-word format (the code asks
   two words since CP-96 and read an empty driver from it), and the
   `image inspect <missing>` fake with invented streams read the file;
   the remaining pre-existing fakes were censused against it (the CP-98
   report): captured real output (the strangers' pull errors, CP-84's
   Compose lines) agrees by construction, and where a fake carries a
   stream the code does not read (`image_identity`'s `--format` triple,
   the `{{json .State}}` object) it is noted there and harmless until
   the code reads more — which is (c)'s moment.
   **(e) The check must name the object it checked** — added at CP-101,
   on the third and fourth occurrence of the class the rule was written
   for. Round six's own harness produced both: a completion matcher that
   looked for `#`/`**` headings counted **0/4** closing sections in a
   transcript that had all four in bare capitals, so a stranger that had
   finished and closed itself was reported as neither; and a validator
   run against the receiver's quarantine **envelope** (`{findings,
   session_result}`) instead of the body inside it, which yielded a
   confident `ADM1/ADM3` verdict about a document nobody had written.
   Both are green checks on the wrong object — a real measurement,
   correctly performed, of something other than the thing claimed. That
   is CP-96's bound that never ran, CP-98's fake nobody measured and
   CP-99's label over an inference, a third and fourth time, and the
   generalisation the earlier clauses were reaching for: **(a)–(d) say a
   fake's shape must be measured rather than remembered; (e) says the
   same of every shape a check assumes** — a heading grammar, a
   document's nesting level, a file's schema — and requires the check to
   record which object it validated, so a passing check cannot be read
   as covering a layer it never opened. Scope, stated because it decides
   who this binds: both CP-101 occurrences were in the **untracked
   round-six rig**, not in `estate/corpus/tests/`, so the suite's own
   machinery did not fail — this rule governs the suite, and the rig is
   the operator's instrument, where the same lesson is recorded rather
   than enforced. The clause is added here anyway because the suite is
   where it would cost most: a test that validates the wrong layer of a
   nested artifact passes for exactly as long as nobody looks.
