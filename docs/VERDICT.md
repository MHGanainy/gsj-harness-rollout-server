...
| 45 | **NEW (CP-51)** — CP-50's operational debt, moved into a row from its report and §7 paragraph: the H200's gitignored instance data (`forgejo/forgejo-data`, `forgejo/.token*`, `mcp-service/data`, `staging/serving/run`) still sits at the pre-CP-50 paths; a `docker compose up` from `estate/` without the four `mv` commands in `estate/README.md` §"Migrating a live checkout past CP-50" mounts an empty `./data` and starts a blank Forgejo — reads as data loss, is a wrong mount. Not verifiable from the repo; CP-50's report is the record | none — operator, on the box | opened CP-50 | every H200 bring-up after CP-50. `waiting-on: the next H200 bring-up — the mv commands first, then compose up shows the existing case repos` |
