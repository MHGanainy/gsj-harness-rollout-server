...
**Status: specified, not implemented.** No implementation conforms to this
document yet: this repository's service returns a bare list of synthetic
decisions (surface level 0 — see §7.6), and `gsj-next` returns
document-level hits without Randnummern (level 1 once it adopts the field
rules; it is not changed by this document). The conformance fixture in
...
|---|---|---|
| **0** | anything else | this repository's service today (a bare list of `{decision_id, court, year, score, text}` over synthetic decisions) |
...
| **2 — Randnummer** | every field of §7.3, with `rn` and `section` on every hit, `text` the whole unit | this repository's service after the building checkpoint; `gsj-next` if and when it adopts §4 |
