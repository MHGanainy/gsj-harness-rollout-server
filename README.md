...
**Release 0.1.18 — Docker pull progress.** Estate pulls now inherit Docker's stdout directly: native layer progress in a terminal, state changes that may stay quiet during a large redirected transfer. Custom host-byte sampling, layer tallies and pull heartbeats are removed. After Docker's ordinary process wait returns, the existing stderr reader gets up to ten seconds to finish, returning sooner when it finishes; inherited stdout adds no separate wait. This grace changes descendant-pipe timing, is not an execution timeout, and does not guarantee partial stderr: capture still waits for the whole read. Caller diagnostics, image policy, readiness and recovery are preserved; retrieval remains 0.5.1.
...
| a stranger can run it from nothing | fresh machine, demo README the only input: clone → pip → estate up → first episode accepted, ≈ 5 minutes wall with the four images already on the daemon — cold, add ~1.6 GiB of pulls (Forgejo ~80 MB, the retrieval service 1.28 GiB, the harness 200 MiB — compressed, from the registry manifests at CP-104; Forgejo's 277 MB is its extracted size, and the demo's Polar image adds ~160 MB): minutes on a fast pipe, most of an hour at ~155 KB/s (round three, 2026-09-07; redirected pulls can remain quiet while a large layer transfers; terminals show Docker's native layer progress) — plus the model endpoint; every image pulls natively on ARM since CP-64 (the two manual pulls this row once needed are history — a round-four stranger caught the stale claim); a copy-on-create daemon (`vfs`) costs a **17× slower sandbox create** (19.4 s against overlay2's 1.1 s on the identical task) and **3.7× the disk** (31 G of data root against 8.4 G, for the same 4.061 GB of images) — measured against an overlay2 control at round five, named by `up` since CP-96 and priced to that pair since CP-99; round four's "minutes per create and ~13 GB per sandbox" was one loaded host, inferred, and is retired | CP-36 |
